...
export async function verifyCapacityOverload({ base, token, signal, sample, upload }) {
  ...
  try {
    for (let index = 0; index < initial.uploads.limit; index++) {
      ...
      const request = http.request(new URL("/api/uploads", base), {
        ...
        headers: {
          ...
          "content-type": "multipart/form-data; boundary=capacity-stalled",
          "content-length": "1024",
        },
      });
      request.on("error", () => {});
    ...
    assert.equal(
      responseOutcome("upload", response, payload),
      "upload_busy",
      "A full admission limit must reject a new upload with UPLOAD_BUSY and Retry-After",
    );
  ...
  return {
    status: "passed",
    heldRequests: initial.uploads.limit,
    rejectedRequests: 1,

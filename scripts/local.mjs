...
export function composeDatabaseUrl(env) {
  ...
  if (!url.username || !url.password || url.pathname === "/")
    throw new Error("PostgreSQL user, password and database must be nonempty");
  return url.href;
}
export function composeArgs(directory, command, profiles = []) {
  if (!["up", "down", "status", "config", "logs", "build"].includes(command))
    throw new Error("Unknown local command");
  if (
    profiles.some(
      (profile) => !["app", "redis", "kafka", "storage", "analytics", "worker"].includes(profile),
    )
  )
    throw new Error("Unknown infrastructure profile");
  ...
  const args = [
    ...
    "--file",
    path.join(directory, "deploy/compose/docker-compose.yml"),
    ...[...selected].flatMap((p) => ["--profile", p]),
  ];
  const actions = {
...
async function main() {
  ...
  const env = {
    ...process.env,
    COMPOSE_PROJECT_NAME: projectName(root),
    COMPOSE_PROFILES: "",
    COMPOSE_DATABASE_URL: composeDatabaseUrl(process.env),
  };
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().catch((error) => {
    console.error(`Local environment failed: ${error.message}`);
    process.exitCode = 1;
  });
}

...
async function probe(mode) {
  if (mode === "control") {
    const kafka = new Kafka({
      clientId: "security-control", brokers: process.env.KAFKA_BROKERS.split(","),
      ssl: { ca: [await readFile(process.env.KAFKA_SSL_CA_FILE, "utf8")] },
      sasl: { mechanism: "plain", username: process.env.KAFKA_SASL_USERNAME, password: process.env.KAFKA_SASL_PASSWORD },
      connectionTimeout: 1500, requestTimeout: 2000, retry: { retries: 0 }, logLevel: logLevel.NOTHING,
    });
    const admin = kafka.admin();
    try {
      await admin.connect();
      await admin.createTopics({ topics: [{ topic: "security-proof", numPartitions: 1, replicationFactor: 1 }] });
    ...
    const result = await runKafkaConsumer({
      topic: "security-proof", groupId: `security-${randomBytes(8).toString("hex")}`,
      maxMessages: 1, maxWaitMs: 15000,
...
if (process.argv[2] === "--probe") {
  try { await probe(process.argv[3]); }
  catch (error) { console.error(`Probe failed: ${error.name}`); process.exitCode = 1; }
} else {
...
async function run() {
  ...
  for (const key of Object.keys(env)) if (/^(DATABASE_|PG|APP_|SESSION_|RATE_LIMIT_|REDIS_|KAFKA_|OUTBOX_|ASYNC_|UPLOAD_|OBJECT_STORAGE_)/.test(key)) delete env[key];
  Object.assign(env, { NODE_ENV: "test", RATE_LIMIT_DRIVER: "memory", UPLOAD_STORAGE_DRIVER: "local", APP_TEMPLATE_WORKER_SKIP_ENV_FILES: "1", KAFKAJS_NO_PARTITIONER_WARNING: "1" });
  ...
  async function command(program, args, options = {}) {
    ...
    return await new Promise((resolve, reject) => {
      const child = spawn(program, args, { cwd: temp, env: options.env || env, stdio: ["ignore", "pipe", "pipe"] });
      children.add(child);
      let output = "";
      let timedOut = false;
      const timer = setTimeout(() => { timedOut = true; child.kill("SIGKILL"); }, options.timeout || 45000);
  ...
  try {
    for (const ca of ["ca", "wrong-ca"]) await command("openssl", ["req", "-x509", "-newkey", "rsa:2048", "-nodes", "-days", "1", "-subj", `/CN=${name}-${ca}`, "-keyout", `${ca}.key`, "-out", `${ca}.crt`]);
    await command("openssl", ["req", "-newkey", "rsa:2048", "-nodes", "-subj", "/CN=localhost", "-keyout", "server.key", "-out", "server.csr"]);
    await writeFile(path.join(temp, "server.ext"), "subjectAltName=DNS:localhost,IP:127.0.0.1\nextendedKeyUsage=serverAuth\n", { mode: 0o600 });
    await command("openssl", ["x509", "-req", "-in", "server.csr", "-CA", "ca.crt", "-CAkey", "ca.key", "-CAcreateserial", "-days", "1", "-extfile", "server.ext", "-out", "server.crt"]);
    await command("openssl", ["pkcs12", "-export", "-in", "server.crt", "-inkey", "server.key", "-certfile", "ca.crt", "-out", "server.p12", "-passout", "env:KAFKA_TEST_STORE_PASSWORD"], { env: { ...env, KAFKA_TEST_STORE_PASSWORD: storePassword } });
    const server = net.createServer();
    await new Promise((resolve, reject) => { server.once("error", reject); server.listen(0, "127.0.0.1", resolve); });
    const port = server.address().port;
    await new Promise((resolve) => server.close(resolve));
    await writeFile(path.join(temp, "server.properties"), [
      ...
      "ssl.keystore.type=PKCS12", "ssl.keystore.location=/security/server.p12",
      `ssl.keystore.password=${storePassword}`, "ssl.client.auth=none",
      "offsets.topic.replication.factor=1", "transaction.state.log.replication.factor=1", "transaction.state.log.min.isr=1",
      "group.initial.rebalance.delay.ms=0", "log.dirs=/tmp/kafka-security-data",
    ].join("\n") + "\n", { mode: 0o600 });
    await command("docker", ["run", "--detach", "--name", name, "--label", `pstack.kafka-security=${id}`, "--user", "0:0", "--publish", `127.0.0.1:${port}:9092`, "--mount", `type=bind,source=${temp},target=/security,readonly`, "--tmpfs", "/tmp", "--entrypoint", "/bin/bash", image, "-ec", "/opt/kafka/bin/kafka-storage.sh format --ignore-formatted --cluster-id MkU3OEVBNTcwNTJENDM2Qk --config /security/server.properties >/dev/null && exec /opt/kafka/bin/kafka-server-start.sh /security/server.properties"]);
    containerCreated = true;
    Object.assign(env, {
      KAFKA_BROKERS: `127.0.0.1:${port}`, KAFKA_CLIENT_ID: name, OUTBOX_PUBLISHER: "kafka",
      KAFKA_SECURITY_PROTOCOL: "SASL_SSL", KAFKA_SSL_CA_FILE: path.join(temp, "ca.crt"),
      KAFKA_SASL_MECHANISM: "plain", KAFKA_SASL_USERNAME: "proof", KAFKA_SASL_PASSWORD: password,
    });
    ...
    for (const entry of ["producer", "consumer", "health", "topic-admin", "control"]) {
      for (const scenario of ["valid", "wrong-ca", "wrong-password"]) {
        ...
        const result = await command(process.execPath, probeArgs(entry), { env: probeEnv, allowFailure: true });
        await writeFile(path.join(evidence, `${entry}-${scenario}.log`), result.output, { mode: 0o600 });
        const passed = !result.timedOut && (scenario === "valid" ? result.code === 0 : result.code !== 0);
        summary.checks.push({ entry, scenario, passed, exitCode: result.code, timedOut: result.timedOut });
        console.log(`${passed ? "PASS" : "FAIL"} ${entry} ${scenario}`);
      }
    }
    assert.ok(summary.checks.every((check) => check.passed), "Kafka security assertions failed");
    summary.status = "passed";
  } catch (error) {
    summary.status = "failed"; summary.error = redact(error.message); process.exitCode = 1; console.error(summary.error);
  } finally {
    if (containerCreated) {
      ...
      const removed = await command("docker", ["rm", "--force", "--volumes", name], { allowFailure: true, cleanup: true });
      if (removed.code !== 0) { summary.cleanupError = removed.output; summary.status = "failed"; process.exitCode = 1; }
    }

...
export function parseArguments(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (!["--file", "--manifest", "--output", "--id"].includes(arg))
      throw new Error(`Unknown argument: ${arg}`);
    const value = args[++i];
    if (!value || value.startsWith("--")) throw new Error(`${arg} requires a value`);
    options[arg.slice(2)] = value;
  }
  if (!["create", "verify", "restore", "help"].includes(command))
    throw new Error("Expected create, verify, restore, or help");
  if (["verify", "restore"].includes(command) && !options.file)
    throw new Error(`${command} requires --file`);
  if (options.id && !/^[a-zA-Z0-9][a-zA-Z0-9._-]{0,79}$/.test(options.id))
...
async function postgresTool(command, args, database, directories = []) {
  ...
  if (process.env.POSTGRES_TOOLS === "docker") {
    ...
    const groups = [...new Set(process.getgroups?.() || [])].flatMap((gid) => [
      "--group-add",
      String(gid),
    ]);
    return run(
  ...
  if (process.env.POSTGRES_TOOLS && process.env.POSTGRES_TOOLS !== "local")
    throw new Error("POSTGRES_TOOLS must be local or docker");
...
async function ledger(client) {
  const rows = (
    await client.query(
      "SELECT id, hash, created_at::text FROM drizzle.drizzle_migrations ORDER BY id",
    )
  ).rows;
  if (!rows.length || rows.some((row) => !/^[a-f0-9]{64}$/.test(row.hash)))
    throw new Error("Database migration ledger is empty or invalid");
  return rows;
}

async function schemaFingerprint(client) {
  const result = await client.query(
    `SELECT n.nspname AS schema, c.relname AS relation, a.attnum, a.attname,
  ...
  const constraints = (
    ...
    WHERE n.nspname=ANY($1) ORDER BY n.nspname,c.relname,con.conname`,
      [schemas],
    )
  ).rows;
  const indexes = (
    await client.query(
      "SELECT schemaname,tablename,indexname,indexdef FROM pg_indexes WHERE schemaname=ANY($1) ORDER BY schemaname,tablename,indexname",
      [schemas],
    )
  ).rows;
...
export async function verifyBackup(options) {
  ...
  const manifest = JSON.parse(
    await readFile(options.manifest || path.join(path.dirname(file), "manifest.json"), "utf8"),
  );
  if (
    manifest.version !== 1 ||
    manifest.format !== "postgres-custom" ||
    manifest.dumpFile !== path.basename(file) ||
    JSON.stringify(manifest.schemas) !== JSON.stringify(schemas) ||
    ...
    manifest.schemaVersion !== manifest.migrations.at(-1).created_at
  )
    throw new Error("Invalid or incomplete backup manifest");
  if (
    (await stat(file)).size !== manifest.dumpBytes ||
    (await sha256(file)) !== manifest.dumpSha256
  )
  ...
  if (!/TABLE DATA drizzle drizzle_migrations /.test(list) || !/TABLE DATA public /.test(list))
    throw new Error("Archive omits application data or migration ledger");
...
export async function createBackup(options, captureSnapshot) {
  ...
  const id =
    options.id ||
    `BKP-${new Date().toISOString().replace(/[:.]/g, "-")}-${randomUUID().slice(0, 8)}`;
  ...
  try {
    ...
    const manifest = {
      ...
      databaseVersion,
      schemaVersion: migrations.at(-1).created_at,
      schemaSha256,
      migrations,
      ledgerSha256: hash(JSON.stringify(migrations)),
      dumpFile: path.basename(file),
      dumpBytes: (await stat(file)).size,
      dumpSha256: await sha256(file),
    };
...
export async function restoreBackup(options, beforeRestore) {
  ...
  try {
    const objects = (
      await client.query(
        "SELECT count(*)::int AS count FROM (SELECT c.relnamespace AS namespace FROM pg_class c UNION ALL SELECT p.pronamespace FROM pg_proc p UNION ALL SELECT t.typnamespace FROM pg_type t) objects JOIN pg_namespace n ON n.oid=objects.namespace WHERE n.nspname=ANY($1)",
        [schemas],
      )
    ).rows[0].count;
    if (objects)
      throw new Error(
        "Restore target must have empty public and drizzle schemas; use a new database",
      );
    await beforeRestore?.(client);
    await restoreArchive(file, list, database);
    const restoredLedger = await ledger(client);
    if (
      hash(JSON.stringify(restoredLedger)) !== manifest.ledgerSha256 ||
      (await schemaFingerprint(client)) !== manifest.schemaSha256
    )
      throw new Error("Restored ledger or schema does not match the backup manifest");
  } finally {
    await client.end();
  }

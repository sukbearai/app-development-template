...
export function parseRetentionArguments(argv, now = new Date()) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (!["--days", "--batch-size"].includes(args[i])) throw new Error(`Unknown argument: ${args[i]}`);
    const key = args[i], value = args[++i];
    if (!/^\d+$/.test(value || "") || !Number.isSafeInteger(Number(value))) throw new Error(`${key} requires an integer`);
    if (key === "--days") options.days = Number(value); else options.batchSize = Number(value);
  }
...
export async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    console.log(JSON.stringify({ dryRun: options.dryRun, before: options.before.toISOString(), batchSize: options.batchSize, counts }));

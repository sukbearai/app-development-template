...
export function parseRetentionArguments(argv, now = new Date()) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (!["--days", "--batch-size", "--session-days"].includes(args[i]))
  ...
  if (!options.days || options.days < 1 || options.days > 36500)
    throw new Error("Explicit retention --days 1..36500 is required");
  if (options.batchSize < 1 || options.batchSize > 1000)
    throw new Error("--batch-size must be 1..1000");
  if (options.sessionDays !== undefined) {
    if (options.sessionDays < 1 || options.sessionDays > 36500)
      throw new Error("--session-days must be 1..36500");
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url))
  main().catch((error) => {
    console.error(
      `History retention failed: ${String(error.message).replace(/postgres(?:ql)?:\/\/[^\s]+/g, "[database URL redacted]")}`,
    );
    process.exitCode = 1;
  });

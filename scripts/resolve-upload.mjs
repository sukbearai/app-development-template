...
export function parseResolveArguments(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (!["--apply", "--confirm-writer-stopped", "--confirm-remote-write-settled"].includes(arg))
      throw new Error(`Unknown argument: ${arg}`);
    options[arg] = true;
  }
  if (
    !options.id ||
    options.id.startsWith("--") ||
    !options["--apply"] ||
    !options["--confirm-writer-stopped"] ||
    !options["--confirm-remote-write-settled"]
  )
    throw new Error(
      "Required: --id INTENT_ID --apply --confirm-writer-stopped --confirm-remote-write-settled",
    );
...
export async function main(argv = process.argv.slice(2)) {
  ...
  try {
    console.log(JSON.stringify(await resolveBlockedUpload(options.id, options.evidence)));
  } finally {
    await closeDatabase();
    await closeRedis();
    closeS3();
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url))
  main().catch((error) => {
    console.error(
      `Upload resolution failed: ${String(error.message).replace(/postgres(?:ql)?:\/\/[^\s]+/g, "[database URL redacted]")}`,
    );
    process.exitCode = 1;
  });

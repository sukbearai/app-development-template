...
export function checkReleaseVersion(config, manifest, packageJson, changelog) {
  ...
  assert.equal(config.$schema, "https://raw.githubusercontent.com/googleapis/release-please/891bcf6253b390e39df9ff3e1c059a836bd39c98/schemas/config.json");
  assert.match(config["bootstrap-sha"], /^[a-f0-9]{40}$/);
  assert.deepEqual(Object.keys(config.packages), ["."]);
  const policy = config.packages["."];
  assert.ok([true, false].includes(policy.prerelease), "prerelease must explicitly select a channel");
  ...
  assert.deepEqual(manifest, { ".": packageJson.version }, "manifest and application version must match");
  if (changelog === undefined) {
    assert.equal(packageJson.version, "0.1.0", "only the unreleased bootstrap version may omit CHANGELOG.md");
  } else {
    const heading = changelog.match(/^#{1,2} \[?(\d+\.\d+\.\d+(?:-rc\.\d+)?)(?:\]|\s|$)/m);
    assert.equal(heading?.[1], packageJson.version, "latest CHANGELOG version must match the application");
  }
...
export function checkReleaseCandidateChannel(config, version) {
  assert.equal(config.packages["."].prerelease, version.includes("-rc."), "candidate version must match the configured release channel");
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    ...
    assert.ok(args.length === 0 || (args.length === 1 && args[0] === "--candidate"), "only --candidate is accepted");

...
for (const key of Object.keys(childEnv)) {
  if (
    /^(PSTACK_VERIFY_|PG[A-Z_]*$|DATABASE_|E2E_|UI_FLOW_|APP_|SESSION_|RATE_LIMIT_|LOGIN_RATE_|REDIS_|OTEL_|KAFKA_|OUTBOX_|ASYNC_|UPLOAD_|OBJECT_STORAGE_|CLICKHOUSE_|BOOTSTRAP_|MONITOR_|METRICS_|SERVICE_|WEB_|SMOKE_|VITE_|PORT$|NODE_OPTIONS$)/.test(
      key,
    )
  )
    delete childEnv[key];
...
Object.assign(childEnv, {
  ...
  UPLOAD_STORAGE_DIR: path.join(output, "uploads"),
...
if (mode === "capacity")
  Object.assign(childEnv, {
    ...
    UPLOAD_STORAGE_SHARED: "false",
    LOGIN_RATE_LIMIT_MAX: String(options.capacity.requests + 10),
...
function command(program, args, { env = childEnv, capture = false, cleanup = false } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code, signal) =>
      code === 0
        ? resolve(stdout.trim())
        : reject(new Error(`${program} ${args[0]} failed (${code ?? signal})`)),
    );
  });
}
...
try {
  ...
  if (mode === "ui" && !production) {
    ...
    if (production) {
      ...
      await command("pnpm", ["build"], {
        env: { ...childEnv, NODE_ENV: "production", APP_ENV: "production" },
      });
    ...
    async function launchServer(overrides = {}) {
      ...
      server = spawn(
        process.execPath,
        [
          ...(production ? [path.join(webRoot, "scripts/start.mjs")] : [vinextCLI, "dev"]),
          "--hostname",
          "127.0.0.1",
          "--port",
          String(port),
      ...
      while (true) {
        ...
        if (server.exitCode !== null || server.signalCode !== null)
          throw new Error("Web server exited before readiness; inspect run.log");
        try {
          const response = await fetch(new URL("/api/hello", childEnv.APP_ORIGIN), {
          ...
          JSON.stringify(
            {
              ...
              hostname: "127.0.0.1",
              port,
              baseURL: childEnv.APP_ORIGIN,
              harnessPid: process.pid,
              pid: server.pid,
              pgid: identity.pgid,
              processStarted: identity.processStarted,
              startedAt,
              ...buildIdentity,
            },
            null,
    ...
    if (mode === "capacity") {
      ...
      capacityResult = await runCapacityWorkload({
        env: childEnv,
        options: options.capacity,
        signal: AbortSignal.any([interruption.signal, AbortSignal.timeout(300_000)]),
      });
      ...
      await command("node", ["scripts/monitor-check.mjs"], {
        env: { ...childEnv, METRICS_URL: new URL("/api/system/metrics", childEnv.APP_ORIGIN).href },
      });
      ...
      if (production) {
        ...
        try {
          await control.query("CREATE DATABASE pstack_restored");
        } finally {
          await control.end();
        }
  ...
  try {
    ...
    cleanupErrors.push("server_cleanup_failed");
  }
  try {
    if (containerCreated)
      await command("docker", ["rm", "-f", "-v", containerName], { capture: true, cleanup: true });
    else {
      const owned = await command(
        ...
        [
          ...
          `name=^/${containerName}$`,
          "--filter",
          "label=pstack-x.test=app",
        ],
        { capture: true, cleanup: true },
      );
      if (owned)
        await command("docker", ["rm", "-f", "-v", owned], { capture: true, cleanup: true });
    }
  } catch {
    cleanupErrors.push("container_cleanup_failed");
  }
  try {
    await releaseProductionLock?.();
  } catch {
    cleanupErrors.push("production_lock_cleanup_failed");
  }
  ...
  if (!failure && JSON.stringify(await sourceIdentity(root)) !== JSON.stringify(verificationSource))
    failure = new Error("Source changed during verification");
  if (capacityResult) {
      ...
      JSON.stringify(
        {
          ...
          status: failure ? "failed" : capacityResult.status,
          cleanupErrors,
        },
        null,
        2,
...
if (failure)
  throw mode === "capacity"
    ? new Error(
        capacityResult?.error ?? `Capacity ${verificationStage} failed; inspect retained evidence`,
      )
    : failure;

...
export function publishOptions(args) {
  const { values } = parseArgs({ args, options: { candidate: { type: 'string' }, evidence: { type: 'string' }, output: { type: 'string' }, repo: { type: 'string' }, apply: { type: 'boolean' }, json: { type: 'boolean' } } });
  assert.ok(values.candidate && values.evidence && values.output && /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(values.repo), 'candidate, evidence, output and repo required');
  const relative = path.relative(process.cwd(), path.resolve(values.output));
  assert.ok(relative.startsWith(`artifacts${path.sep}`), 'Publish output must be within artifacts/');
  return values;
}
export function inspectRegistryManifest(bytes, expectedId) {
  const manifest = JSON.parse(bytes.toString('utf8'));
  assert.ok(['application/vnd.oci.image.manifest.v1+json', 'application/vnd.docker.distribution.manifest.v2+json'].includes(manifest.mediaType), 'Only verified single-platform image manifests may be published');
  assert.equal(manifest.config?.digest, expectedId, 'Registry image differs from tested image');
  return `sha256:${sha256(bytes)}`;
}
function exec(program, args) { return execFileSync(program, args, { maxBuffer: 64 * 1024 * 1024, stdio: ['ignore', 'pipe', 'pipe'] }); }
function ghJson(endpoint) { return JSON.parse(exec('gh', ['api', endpoint]).toString('utf8')); }
export function archiveArguments(archive, fileList) {
  return ['--sort=name', '--mtime=UTC 1970-01-01', '--owner=0', '--group=0', '--numeric-owner', '--mode=0644', '--use-compress-program=gzip -n', '-cf', archive, '-T', fileList];
}
...
async function uploadAsset(repo, release, file, name) {
  ...
  if (existing) {
    assert.equal(existing.digest, `sha256:${reference.sha256}`, 'Existing release asset differs; refusing overwrite');
  ...
  const uploaded = ghJson(`repos/${repo}/releases/${release.id}`).assets.find(asset => asset.name === name);
  assert.equal(uploaded?.digest, `sha256:${reference.sha256}`, 'Uploaded asset digest does not match');
...
export async function publishRelease(options) {
  ...
  const tag = `v${version}`;
  if (!options.apply) return { status: 'passed', data: { version, tag, source: candidate.source, apply: false } };
  const releases = JSON.parse(exec('gh', ['api', '--paginate', '--slurp', `repos/${options.repo}/releases?per_page=100`]).toString('utf8')).flat();
  ...
  for (const role of ['web', 'worker']) {
    ...
    const existing = spawnSync('docker', ['buildx', 'imagetools', 'inspect', reference, '--raw'], { encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
    let raw;
    if (existing.status === 0) {
      raw = Buffer.from(existing.stdout);
      inspectRegistryManifest(raw, image.id);
    } else {
      assert.ok(!existing.error && /manifest unknown|not found|NAME_UNKNOWN|MANIFEST_UNKNOWN/i.test(existing.stderr) && !/unauthorized|denied|forbidden/i.test(existing.stderr), 'Cannot establish registry tag absence');
      exec('docker', ['image', 'load', '--input', path.resolve(root, image.archive.path)]);
      const actual = exec('docker', ['image', 'inspect', '--format', '{{.Id}}', image.id]).toString().trim();
    ...
    const exact = exec('docker', ['buildx', 'imagetools', 'inspect', `${repository}@${digest}`, '--raw']);
    ...
    receipt.images[role] = { reference: `${repository}@${digest}`, id: image.id, platform: image.platform, manifest: await evidenceReference(root, manifestFile) };
  }
  const receiptFile = path.join(output, 'registry.json');
  await writeFile(receiptFile, `${JSON.stringify(receipt, null, 2)}\n`);
  const manifestFile = path.join(output, 'release.json');
  await createReleaseManifest({ root, candidateFile, evidenceFile, receiptFile, outputFile: manifestFile });
  const files = new Set([path.relative(root, manifestFile), path.relative(root, candidateFile), path.relative(root, evidenceFile), path.relative(root, receiptFile), candidate.verification.path,
    ...Object.values(candidate.images).map(image => image.archive.path), ...Object.values(receipt.images).map(image => image.manifest.path),
    ...index.checks.flatMap(check => check.evidence.map(reference => reference.path))]);
  const fileList = path.join(output, 'evidence-files.txt');
  for (const file of files) assert.ok(!file.includes('\n') && !file.startsWith('-'), 'Unsafe archive entry');
  ...
  assert.equal(ghJson(`repos/${options.repo}/commits/${tag}`).sha, candidate.source.gitSha, 'Tag moved during publication');
  exec('gh', ['api', '--method', 'PATCH', `repos/${options.repo}/releases/${release.id}`, '-F', 'draft=false', '-F', `prerelease=${channel === 'rc'}`, '-f', `make_latest=${channel === 'stable' ? 'true' : 'false'}`]);
  return { status: 'passed', evidence: path.relative(root, manifestFile), data: { version, tag, releaseId: release.id, images: receipt.images } };
}
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  let options;
  try { options = publishOptions(process.argv.slice(2)); }
  catch { printCommandResult(commandResult({ command: 'release:publish', status: 'invalid', errorCode: 'invalid_arguments' }), process.argv.includes('--json')); }
  if (options) {
    try { printCommandResult(commandResult({ command: 'release:publish', ...await publishRelease(options) }), options.json); }
    catch { printCommandResult(commandResult({ command: 'release:publish', status: 'failed', errorCode: 'publication_failed' }), options.json); }
  }
}

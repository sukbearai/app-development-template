...
export function verificationOptions(args) {
  ...
  try {
    ({ values } = parseArgs({ args, options: {
      production: { type: 'boolean' }, ui: { type: 'boolean' }, capacity: { type: 'boolean' },
      ...Object.fromEntries(Object.keys(bounds).map(key => [key, { type: 'string' }])),
    } }));
  } catch {
    throw new Error('Invalid verification arguments. Use --production --capacity with optional --concurrency, --requests and --upload-bytes.');
  }
  ...
  for (const [key, [fallback, min, max]] of Object.entries(bounds)) {
    if (values[key] !== undefined && !values.capacity) throw new Error(`--${key} requires --capacity`);
  ...
  if (capacity.requests * capacity.uploadBytes > 1024 ** 3) throw new Error('Total requested upload bytes must not exceed 1 GiB');
  return { production: values.production === true, mode: values.capacity ? 'capacity' : values.ui ? 'ui' : 'api', capacity };
}

export async function refuseCapacityEnvFiles(root) {
  for (const directory of [root, path.join(root, 'apps/web')]) {
    const names = await readdir(directory);
    if (names.some(name => /^\.env(?:\.(?:local|production|test|development)(?:\.local)?)?$/.test(name)))
...
export async function acquireProductionLock(root) {
  ...
  try { handle = await open(lock, 'wx', 0o600); }
    ...
    if (error.code === 'EEXIST') throw new Error('Production verification already owns this checkout, or its lock requires manual inspection');
    throw error;
  }
  ...
  return async () => { await handle.close(); await rm(lock); };
}

export function ownedCapacityTarget(origin, databaseURL) {
  const web = new URL(origin);
  const database = new URL(databaseURL);
  if (web.protocol !== 'http:' || web.hostname !== '127.0.0.1' || !web.port || web.pathname !== '/' || web.username || web.password || web.search || web.hash)
    throw new Error('Capacity requires the owned loopback HTTP target');
  if (database.protocol !== 'postgresql:' || database.hostname !== '127.0.0.1' || !database.port || database.pathname !== '/pstack_test' || database.username !== 'postgres' || !database.password || database.search || database.hash)

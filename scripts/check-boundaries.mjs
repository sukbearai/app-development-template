...
for (const file of files) {
  ...
  function visit(node) {
    if ((ts.isImportDeclaration(node) || ts.isExportDeclaration(node)) && node.moduleSpecifier && ts.isStringLiteral(node.moduleSpecifier)) {
      if (!('importClause' in node && node.importClause?.isTypeOnly) && !node.isTypeOnly) imports.push(node.moduleSpecifier.text);
    }
...
function resolve(source, dependency) {
  ...
  else if (dependency.startsWith('@pstack/contracts')) base = path.join(root, 'packages/contracts/src', dependency.split('/')[2] || 'index');
  if (!base) return undefined;
  return [base, `${base}.ts`, `${base}.tsx`, path.join(base, 'index.ts')].find(candidate => modules.has(candidate));
}
...
for (const [file, module] of modules) {
  for (const dependency of module.imports) {
    const target = resolve(file, dependency);
    if (!target) continue;
    if (file.includes('/packages/contracts/')) assert.ok(target.includes('/packages/contracts/'), `${file}: contracts imports ${target}`);
    if (file.includes('/packages/database/')) assert.ok(!target.includes('/packages/server/') && !target.includes('/apps/web/'), `${file}: database imports ${target}`);
    if (file.includes('/packages/server/')) assert.ok(!target.includes('/apps/web/'), `${file}: server imports ${target}`);
  }

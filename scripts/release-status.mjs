...
export function validateReleaseIdentity(release, tag, expectedSha, tagCommit) {
  ...
  assert.equal(
    release.prerelease,
    match[1].includes("-rc."),
    "GitHub prerelease flag differs from version",
  );
  ...
  if (expectedSha !== undefined) {
    ...
    assert.equal(
      ...
      expectedSha,
      "draft differs from the original candidate SHA",
    );
  }
...
export async function resolveReleaseStatus(api, repository, tag, expectedSha) {
  ...
  assert.equal(
    matches.length,
    1,
    "expected exactly one matching release; retry requires its original draft",
  );
  ...
  assert.ok(
    ...
    "candidate is not an ancestor of main",
  );
  return result;
}
...
export async function preflightRelease(api, repository) {
  const pulls = await api(
    `repos/${repository}/pulls?state=closed&base=main&per_page=100&sort=updated&direction=desc`,
  );
  const pending = pulls.filter(
    (pull) => pull.merged_at && pull.labels.some((label) => label.name === "autorelease: pending"),
  );
  ...
  for (const pull of pending) {
    ...
    const file = await api(
      `repos/${repository}/contents/package.json?ref=${pull.merge_commit_sha}`,
    );
    ...
    if (existing !== null)
      assert.equal(existing, pull.merge_commit_sha, "existing tag points to another commit");
    const matches = releases.filter((release) => release.tag_name === tag);
    assert.equal(
      matches.length,
      0,
      "release already exists; use explicit retry_tag to resume the original draft",
    );
  }
...
export async function resolveRecoveryArtifact(api, repository, tag, runId, attempt) {
  ...
  assert.ok(
    ...
    "Recovery cannot use pull request execution",
  );
...
function githubApi(endpoint) {
  try {
    ...
    if (endpoint.includes("/artifacts?"))
      return { artifacts: pages.flatMap((page) => page.artifacts) };
    return pages[0];
  } catch (error) {
...
export function parseReleaseStatusArgs(args) {
  ...
  assert.ok(
    ["preflight", "resolve", "recovery"].includes(command),
    "expected preflight, resolve or recovery",
  );
  const allowed =
    command === "recovery"
      ? ["--repo", "--tag", "--run-id", "--attempt"]
      : ["--repo", "--tag", "--sha"];
  const options = {};
  for (let index = 0; index < flags.length; index += 2) {
    const key = flags[index];
    assert.ok(
      allowed.includes(key) && !Object.hasOwn(options, key),
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    ...
    if (args.command === "preflight") result = await preflightRelease(githubApi, args.repository);
      ...
      result = await resolveRecoveryArtifact(
        githubApi,
        args.repository,
        args.tag,
        args.runId,

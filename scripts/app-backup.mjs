...
import { copyFile, lstat, mkdir, open, readFile, readdir, rename, writeFile } from "node:fs/promises";
...
async function syncFile(file) { await syncPath(file); await syncPath(path.dirname(file)); }
...
export function parseArguments(argv) {
  ...
  if (!["create", "verify", "restore"].includes(command)) throw new Error("Expected create, verify, or restore");
...
export async function references(client) {
  const rows = (await client.query(`SELECT f.id, f.storage_key AS key, f.size_bytes::text AS bytes,
    f.mime_type AS "contentType", i.id AS "intentId", i.provider, i.storage_location AS location, i.state
    FROM app_file_assets f LEFT JOIN app_upload_intents i ON i.storage_key=f.storage_key ORDER BY f.id`)).rows;
  const seen = new Set();
  for (const row of rows) {
    if (!keyPattern.test(row.key) || !["local", "s3"].includes(row.provider) || row.state !== "committed" ||
        !row.location || row.location === "legacy-unbound" || !/^\d+$/.test(row.bytes) ||
        !Number.isSafeInteger(Number(row.bytes)) || seen.has(row.key)) throw new Error("File has missing, duplicate or unbound committed upload intent");
    seen.add(row.key);
  }
  const orphans = (await client.query(`SELECT count(*)::int AS n FROM app_upload_intents i
    LEFT JOIN app_file_assets f ON f.storage_key=i.storage_key WHERE i.state='committed' AND f.id IS NULL`)).rows[0].n;
...
export function storage(env = process.env) {
  ...
  return {
    location(provider) {
      if (provider === "local") { if (!local) throw new Error("Explicit UPLOAD_STORAGE_DIR is required"); return local; }
      s3();
    ...
    async download(row, destination) {
      if (row.provider === "local") {
        ...
        if (!(await lstat(source)).isFile()) throw new Error("Managed object must be a regular file");
    ...
    async assertEmpty(provider) {
      ...
      if (provider === "local") {
        try { if (!(await lstat(local)).isDirectory() || (await readdir(local)).length) throw new Error("Local restore target must be an empty real directory"); }
        catch (error) { if (error.code !== "ENOENT") throw error; }
      } else {
        s3();
        const result = await send(new sdk.ListObjectsV2Command({ Bucket: bucket, MaxKeys: 1 }));
        if (result.KeyCount || result.Contents?.length || result.IsTruncated) throw new Error("S3 restore target bucket must be empty");
      }
    },
    async upload(row, source) {
      if (row.provider === "local") {
        ...
        try {
          await Promise.race([readFailure, send(new sdk.PutObjectCommand({ Bucket: bucket, Key: row.key, Body: body,
            ContentLength: Number(row.bytes), ContentType: row.contentType, IfNoneMatch: "*" }))]);
    ...
    async verify(row) {
      ...
      else { s3(); stream = (await send(new sdk.GetObjectCommand({ Bucket: bucket, Key: row.key }))).Body; }
      const digest = createHash("sha256"); let bytes = 0;
      for await (const chunk of stream) { digest.update(chunk); bytes += chunk.length; }
      if (bytes !== Number(row.bytes) || digest.digest("hex") !== row.sha256) throw new Error(`Restored object checksum differs: ${row.key}`);
    },
...
async function checkObject(file, row) {
  ...
  if (!info.isFile() || info.size !== Number(row.bytes) || await sha256(file) !== row.sha256) throw new Error(`Object checksum or size mismatch: ${row.key}`);
}
  ...
  try {
    ...
    await createBackup({ output: path.join(output, "database"), id: "database" }, async (client) => {
      for (const row of await references(client)) {
        ...
        await syncFile(file);
        if ((await lstat(file)).size !== Number(row.bytes)) throw new Error(`Object size differs from database: ${row.key}`);
...
export async function verifyBundle(directory) {
  ...
  if (actual.length !== seen.size || actual.some((key) => !seen.has(key))) throw new Error("Bundle objects differ from manifest");
...
export async function restoreBundle(options) {
  ...
  try {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error("Restored database references differ from bundled objects; discard this isolated target");
    for (const row of manifest.objects) {
      await target.upload(row, path.join(directory, "objects", row.key));
      await target.verify(row);
    }
    for (const row of manifest.objects) {
      const changed = await client.query("UPDATE app_upload_intents SET storage_location=$1 WHERE id=$2 AND state='committed' AND storage_location=$3", [target.location(row.provider), row.intentId, row.location]);
      if (changed.rowCount !== 1) throw new Error("Committed upload changed during restore");
    }
    await client.query(`INSERT INTO app_audit_logs (id,action,target_type,trace_id,metadata)
  ...
  } finally { await client.end(); target.close(); }
}

export async function main(argv = process.argv.slice(2)) {
  loadEnvironment(root); process.umask(0o077);
  const options = parseArguments(argv);
  if (options.command === "create") return createBundle(options);
  if (options.command === "restore") return restoreBundle(options);
  await verifyBundle(options.directory); console.log("Bundle manifests, database archive and every object checksum verified");
}
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) main().catch((error) => {
  console.error(`Application backup failed: ${String(error.message).replace(/postgres(?:ql)?:\/\/[^\s]+/g, "[database URL redacted]")}`);
  process.exitCode = 1;
});

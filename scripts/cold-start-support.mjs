...
export function coldStartOptions(args) {
  if (args.some((arg) => !["--json", "--help"].includes(arg)) || args.length > 1) {
    throw Object.assign(new Error("Use --json or --help; no other arguments are accepted."), {
...
export async function exportCheckout(root, destination) {
  const files = execFileSync(
    ...
    { cwd: root, env: isolatedEnvironment() },
  )
    .toString()
    .split("\0")
    .filter(Boolean);
  ...
  for (const file of [...new Set(files)].sort()) {
    if (
      file
        .split("/")
        .some((part) => ["node_modules", ".git", ".verification", "artifacts"].includes(part))
    )
      continue;
    ...
    if (!info) continue;
    if (!info.isFile())
      throw Object.assign(new Error(`Source export requires regular files: ${file}`), {
        code: "UNSAFE_SOURCE",
      });
    ...
    hash
      .update(file)
      .update("\0")
      .update(await readFile(path.join(destination, file)))
      .update("\0");
    copied++;
  }
  return {
...
export function commandRunner(cwd, env, write) {
  ...
  function launch(program, args, cleanup = false) {
    ...
    const done = new Promise((resolve, reject) => {
      child.once("error", (error) => {
        ...
        reject(
          Object.assign(new Error(`${program} cannot start. Install it and verify PATH.`), {
            code: error.code === "ENOENT" ? "DEPENDENCY_MISSING" : "PROCESS_START_FAILED",
          }),
        );
      });
      child.once("close", (code, signal) => {
        ...
        if (code === 0) resolve(output.trim());
          ...
          reject(
            Object.assign(new Error(`${program} exited ${code ?? signal}; inspect run.log.`), {
              code: interrupted ? "INTERRUPTED" : "COMMAND_FAILED",
            }),
          );
      });
    });
  ...
  return {
    launch,
    run: (program, args, cleanup) => launch(program, args, cleanup).done,
    stop,
    interrupt() {
      interrupted = true;
      return stop();

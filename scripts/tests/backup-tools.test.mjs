...
test("Docker PostgreSQL tools retain supplementary groups for group-readable TLS keys", { skip: !process.getgroups }, async (t) => {
  ...
  try {
    ...
    await restoreArchive(path.join(directory, "archive.dump"), "", "postgres://app@localhost/backup?sslkey=client.key");

...
const { buildRuntimePlanFromEnv } = await tsImport("../../packages/server/src/async-runtime-health-service.ts", import.meta.url);
const { buildAsyncRuntimePlan } = await tsImport("../../services/worker/src/async-runtime.ts", import.meta.url);
...
test("Compose diagnostics match worker configuration without enabling Kafka readiness on Web", () => {
  const source = execFileSync("docker", ["compose", "--env-file", ".env.example", "-f", "deploy/compose/docker-compose.yml", "--profile", "*", "config", "--format", "json"], {
    cwd: new URL("../..", import.meta.url), encoding: "utf8",
    env: { ...process.env, COMPOSE_DATABASE_URL: "postgres://test:test@postgres/test", ASYNC_RUNTIME_TOPICS: "app.tasks,custom.events" },
  });

...
import { sourceIdentity, createEvidenceRun, evidenceReference, writeEvidenceIndex, verifyEvidence } from '../verification-evidence.mjs';
...
test('source identity includes dirty untracked files and tracked deletion', async t => {
  ...
  assert.equal(after.dirty, true); assert.notEqual(after.sourceSha256, before.sourceSha256);
...
test('evidence verifies actual bytes and rejects tampering or another source', async t => {
  ...
  const check = { name: 'unit', status: 'passed', startedAt: run.startedAt, finishedAt: run.startedAt, durationMs: 0, evidence: [await evidenceReference(root, log)] };
  ...
  await assert.rejects(verifyEvidence(file, root, { ...run.source, dirty: true }), /source mismatch/);
...
test('not-run and failed checks cannot form a successful index; symlink escape is refused', async t => {
  const root = await fixture(t); const run = await createEvidenceRun(root);
  const log = path.join(run.output, 'skip.log'); await writeFile(log, 'not run');
  const check = { name: 'unit', status: 'not-run', startedAt: run.startedAt, finishedAt: run.startedAt, durationMs: 0, evidence: [await evidenceReference(root, log)] };
  ...
  await assert.rejects(evidenceReference(root, path.join(run.output, 'outside')), /within its root/);
});
test('logged command invokes actual process, preserves failure, and returns stable envelope', async t => {
  const root = await fixture(t);
  const result = await runLogged({ command: process.execPath, args: ['-e', 'process.stdout.write("proof"); process.exitCode=7'], cwd: root, logFile: path.join(root, 'run.log') });
  assert.equal(result.code, 7);
  assert.equal(commandExitCode('invalid'), 2);
  assert.equal(commandResult({ command: 'test', runId: 'fixture', status: 'failed', errorCode: 'check_failed' }).schemaVersion, 1);
...
test('unavailable log prevents the child from starting', async t => {
  ...
  await assert.rejects(runLogged({ command: process.execPath, args: ['-e', 'require("node:fs").writeFileSync(process.argv[1], "started")', marker], cwd: root, logFile }), /EEXIST/);
...
test('interrupted child that ignores SIGTERM is killed within the explicit grace', async t => {
  ...
  const run = runLogged({ command: process.execPath, args: ['-e', 'process.on("SIGTERM",()=>{});require("node:fs").writeFileSync(process.argv[1],"ready");setInterval(()=>{},1000)', marker],
    cwd: root, logFile: path.join(root, 'ignored-stop.log'), signal: controller.signal, stopGraceMs: 50 });
  for (let attempts = 0; ; attempts++) {
    try { await access(marker); break; } catch (error) { if (error.code !== 'ENOENT' || attempts > 100) throw error; await delay(10); }
  }
  controller.abort();
  const result = await run;
  assert.equal(result.signal, 'SIGKILL'); assert.equal(result.interrupted, true);
});

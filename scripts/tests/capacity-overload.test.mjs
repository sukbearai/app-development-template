...
async function admissionServer(operation) {
  ...
  const server = http.createServer((request, response) => {
    if (active === 2) {
      ...
      response.end(JSON.stringify({ traceId: 'trace_capacity_test', error: { code: 'UPLOAD_BUSY', message: 'Upload slots occupied' } }));
      return;
    }
    active++;
  ...
  const upload = async () => {
    const response = await fetch(`${base}/api/uploads`, { method: 'POST', body: 'x', signal: AbortSignal.timeout(1000) });
    return { response, payload: await response.json() };
  };
  try { await operation({ base, token: 'test-only', sample, upload, signal: AbortSignal.timeout(3000) }, () => active); }
  finally {
...
test('overload probe holds complete admission, verifies rejection, and releases requests', async () => {
  await admissionServer(async (input, active) => {
    assert.deepEqual(await verifyCapacityOverload(input), { status: 'passed', heldRequests: 2, rejectedRequests: 1, activeAfterRelease: 0 });
...
test('overload probe closes held sockets when rejection validation fails', async () => {
  await admissionServer(async (input, active) => {
    ...
    while (active() > 0 && Date.now() < deadline) await new Promise(resolve => setTimeout(resolve, 10));
    assert.equal(active(), 0);
  });
});

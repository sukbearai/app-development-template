...
test("backup rejects invalid S3 addressing configuration before storage access", () => {
  ...
  for (const value of ["true", "false", "1", "0", undefined]) {
    ...
    try {
      assert.equal(target.location("s3"), "https://s3.invalid/app-files");
    } finally {
      target.close();
    }
  }
  for (const value of ["yes", "", "FALSE"])
    assert.throws(() =>
      storage({ ...config, OBJECT_STORAGE_FORCE_PATH_STYLE: value }).location("s3"),
    );
});
  ...
  try {
    for (const [value, expected] of [
      ...
      const config = {
        ...
        OBJECT_STORAGE_ACCESS_KEY: "test",
        OBJECT_STORAGE_SECRET_KEY: "test",
        OBJECT_STORAGE_FORCE_PATH_STYLE: value,
      };
...
test("retention is dry-run by default with explicit bounded age and batch", () => {
  ...
  for (const args of [
    [],
    ["--days", "0"],
    ["--days", "1", "--batch-size", "1001"],
    ["--days", "1.5"],
    ["--apply"],
  ])
    assert.throws(() => parseRetentionArguments(args));
});
...
test("blocked upload resolution requires explicit write and remote outcome evidence", async () => {
  ...
  assert.throws(() =>
    parseResolveArguments([
      "--id",
      "intent",
      "--confirm-writer-stopped",
      "--confirm-remote-write-settled",
    ]),
  );
  assert.deepEqual(
    parseResolveArguments([
    ...
    { id: "intent", evidence: { writerStopped: true, remoteWriteSettled: true } },
  );
});
...
test("session retention requires separate bounded opt-in and keeps the history age", () => {
  ...
  assert.equal(options.dryRun, true);
  for (const age of ["0", "-1", "1.5", "36501", "NaN", "9007199254740992"])
    assert.throws(() => parseRetentionArguments(["--days", "30", "--session-days", age]));

...
test('RC increments and promotion use the version tool ordering', () => {
  const compatibility = { migrationLedgerSha256: 'a'.repeat(64), recoveryProtocol: 'pstack-recovery-v2', rollbackVersions: [] };
  ...
  assert.throws(() => verifyTransition(release('1.2.3-rc.2'), release('1.2.3-rc.1'), false), /rollback/);
...
async function publishedApi(fixture, mutation = null) {
  ...
  const remote = { id: 42, draft: false, tag_name: fixture.release.tag, published_at: fixture.release.createdAt, assets: [{ name: 'release.json', size: ref.bytes, digest: `sha256:${ref.sha256}` }] };
  ...
  return endpoint => endpoint.includes('/commits/') ? { sha: mutation === 'SHA' ? 'f'.repeat(40) : fixture.release.source.gitSha } : remote;
}
test('published release produces digest-pinned plan without deployment writes', async t => {
  const f = await releaseFixture(t);
  const plan = await deploymentPlan(f.root, f.outputFile, 'example/pstack', null, false, await publishedApi(f));
...
for (const mutation of ['draft', 'asset', 'missing asset', 'SHA']) {
  test(`deployment refuses published release with invalid ${mutation}`, async t => {
    const f = await releaseFixture(t);
    await assert.rejects(deploymentPlan(f.root, f.outputFile, 'example/pstack', null, false, await publishedApi(f, mutation)));
  });
}
...
test('downloaded archive tampering prevents a deployment plan', async t => {
  ...
  await assert.rejects(deploymentPlan(f.root, f.outputFile, 'example/pstack', null, false, await publishedApi(f)), /content mismatch/);
});

...
import { createRelease, createReleaseManifest, verifyCandidateInputs, verifyRelease } from '../release-manifest.mjs';
...
for (const mutation of ['missing gate', 'dirty source', 'wrong source', 'failed summary', 'archive tamper', 'false receipt', 'wrong config', 'missing evidence']) {
  test(`release refuses ${mutation}`, async t => {
    const f = await releaseFixture(t);
    if (mutation === 'missing gate') { f.evidence.checks.pop(); await f.put('artifacts/index.json', f.evidence); }
    if (mutation === 'dirty source') { f.candidate.source.dirty = true; await f.put('artifacts/candidate.json', f.candidate); }
    if (mutation === 'wrong source') { f.evidence.source.gitSha = 'f'.repeat(40); await f.put('artifacts/index.json', f.evidence); }
    ...
    if (mutation === 'false receipt') { f.receipt.images.web.reference = `ghcr.io/example/web@sha256:${'f'.repeat(64)}`; await f.put('artifacts/registry.json', f.receipt); }
    if (mutation === 'wrong config') {
      const name = f.receipt.images.web.manifest.path;
      await f.put(name, { schemaVersion: 2, mediaType: 'application/vnd.oci.image.manifest.v1+json', config: { digest: `sha256:${'f'.repeat(64)}` }, layers: [] });
    ...
    if (mutation === 'missing evidence') { f.evidence.checks[0].evidence[0].path = 'artifacts/missing.log'; await f.put('artifacts/index.json', f.evidence); }
...
test('release refuses coherent failed or incomplete raw container evidence', async t => {
  ...
  await assert.rejects(createRelease(f.root, f.candidateFile, f.evidenceFile, f.receiptFile), /Container verification failed/);
  ...
  await f.put('artifacts/candidate.json', f.candidate);
  await f.put('artifacts/index.json', f.evidence);
  await assert.rejects(createRelease(f.root, f.candidateFile, f.evidenceFile, f.receiptFile), /Missing container behavior check/);
});

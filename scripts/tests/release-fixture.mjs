...
import { createReleaseManifest, mandatoryReleaseChecks, migrationLedgerPath, requiredContainerChecks } from '../release-manifest.mjs';
...
export async function releaseFixture(t) {
  ...
  const put = async (name, value) => { await writeFile(path.join(root, name), JSON.stringify(value)); return path.join(root, name); };
  ...
  for (const role of ['web', 'worker']) {
    ...
    const manifest = { schemaVersion: 2, mediaType: 'application/vnd.oci.image.manifest.v1+json', config: { digest: ids[role] }, layers: [] };
    const name = `artifacts/${role}-manifest.json`;
    await put(name, manifest);
    const manifestRef = await ref(name);
    published[role] = { reference: `ghcr.io/example/pstack-${role}@sha256:${manifestRef.sha256}`, id: ids[role], platform: 'linux/arm64', manifest: manifestRef };
  }
  const summaryName = 'artifacts/summary.json';
  await put(summaryName, { source, status: 'passed', checks: requiredContainerChecks, images: { web: { id: ids.web, platform: 'linux/arm64' }, worker: { id: ids.worker, platform: 'linux/arm64' } } });
  const candidate = { schemaVersion: 1, source, createdAt: new Date().toISOString(), images, verification: await ref(summaryName) };
  ...
  const evidence = { schemaVersion: 1, runId: randomUUID(), source, startedAt: candidate.createdAt, finishedAt: candidate.createdAt, environment: { node: process.version, platform: 'linux', arch: 'arm64', ciRun: '1234' }, status: 'passed', checks: [] };
  for (const name of mandatoryReleaseChecks) evidence.checks.push({ name, status: 'passed', startedAt: candidate.createdAt, finishedAt: candidate.createdAt, durationMs: 1, evidence: [await ref(name === 'test:containers' ? summaryName : 'artifacts/check.log')] });

...
test("package boundary gate rejects builtins and relative cross-layer imports", async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(fixture, "packages/server/src/index.ts"),
    ...
    for (const dependency of ["@pstack/kafka", "../../../packages/kafka/src/index"]) {
      await writeFile(
        path.join(fixture, "apps/web/components/client.tsx"),
        `"use client"; import "${dependency}";`,
      );
      assert.throws(run, /browser/);
    }
    await writeFile(
      path.join(fixture, "apps/web/components/client.tsx"),
      '"use client"; import "../lib/kafka";',
    );
    await writeFile(
      path.join(fixture, "apps/web/lib/kafka.ts"),
      'export * from "../../../packages/kafka/src/index";',
    );
    assert.throws(run, /browser/);
    ...
    for (const dependency of ["@pstack/kafka", "../../kafka/src/index"]) {
      await writeFile(
        path.join(fixture, "packages/contracts/src/index.ts"),
        `import "${dependency}";`,
      );

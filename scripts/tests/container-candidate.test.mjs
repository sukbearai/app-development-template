...
test("export rejects outside, unignored and reused paths before Docker", async (t) => {
  ...
  for (const args of [
    ...
    ["--export", "../outside"],
    ["--export", "docs"],
    ["--export", "artifacts/run"],
  ])
    assert.throws(() => containerOptions(args, root));
...
test("candidate binds both saved image archives and raw verification bytes", async (t) => {
  ...
  await writeFile(summaryFile, '{"status":"passed"}\n');
  const images = {};
  for (const role of ["web", "worker"]) {
    await writeFile(path.join(output, `${role}.tar`), role);
    images[role] = { id: `sha256:${sha256(role)}`, platform: "linux/arm64" };
  }
  const file = await writeContainerCandidate({
    root,
    output,
    summaryFile,
    images,
    source: { gitSha: "a".repeat(40), sourceSha256: "b".repeat(64), dirty: false },
  });
  ...
  await assert.rejects(
    writeContainerCandidate({ root, output, summaryFile, images, source: candidate.source }),
    /EEXIST/,
  );
});

...
const operationIds = {
  login: 'postApiAuthLogin', read: 'getApiAdminRoles', write: 'postApiAdminRoles',
  upload: 'postApiUploads', metrics: 'getApiSystemMetrics',
};
...
export function summarizeRequests(observations, elapsedMs) {
  ...
  return {
    ...
    latencyMsByOutcome: Object.fromEntries(['success', 'upload_busy', 'failed'].map(outcome => [outcome, latencyPercentiles(observations.filter(item => item.outcome === outcome))])),
...
export function summarizeMetrics(samples) {
  ...
  return {
    ...
    uploadRejectedDelta: samples.length ? samples.at(-1).uploads.rejectedTotal - samples[0].uploads.rejectedTotal : null,
  };
}

export function assessCapacityResponse(operation, response, body) {
  let payload;
  try { payload = parseCapacityResponse(operation, response.status, body); }
  catch (error) { return { outcome: 'failed', diagnostic: capacityFailure('response_contract', error) }; }
  if (operation === 'upload' && response.status === 503 && payload?.error?.code === 'UPLOAD_BUSY' && /^[1-9]\d*$/.test(response.headers.get('retry-after') ?? ''))

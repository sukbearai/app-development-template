...
export function commandResult({ command, runId = randomUUID(), status, errorCode = null, evidence = null, data = null }) {
...
export async function runLogged({ command, args, cwd, logFile, signal, env = process.env, stopGraceMs = 45000 }) {
  if (!Number.isSafeInteger(stopGraceMs) || stopGraceMs < 1) throw new Error('Invalid process stop grace');
  ...
  const forward = (bytes) => { log.write(bytes); process.stderr.write(bytes); };
  child.stdout.on('data', forward);
  child.stderr.on('data', forward);
  const stop = () => {
    if (!child.pid || child.exitCode !== null || child.signalCode !== null) return;
    try { process.kill(-child.pid, 'SIGTERM'); } catch (error) { if (error.code !== 'ESRCH') failure = error; }
    forceStop ??= setTimeout(() => { try { process.kill(-child.pid, 'SIGKILL'); } catch (error) { if (error.code !== 'ESRCH') failure = error; } }, stopGraceMs);
  };

...
if (process.argv[2] === "--interrupt-restore") {
  ...
  Client.prototype.query = function (...args) {
    ...
    if (typeof args[0] === "string" && args[0].startsWith("INSERT INTO app_kafka_recovery"))
      process.kill(process.pid, "SIGKILL");
    return originalQuery.apply(this, args);
  };
  const { restoreBundle } = await import("./app-backup.mjs");
  await restoreBundle({
    directory: process.env.RECOVERY_TEST_BUNDLE,
    confirm: true,
    recoverKafka: true,
  });
  ...
  try {
    console.log(JSON.stringify(await store.dueMessages(process.env.KAFKA_CONSUMER_GROUP_ID)));
  } finally {
    await closeDatabase();
  }
  ...
  try {
    ...
    consumer = runKafkaConsumer({
      topics: asyncRuntimeTopics(),
      groupId: group,
      recovery,
      brokers: process.env.KAFKA_BROKERS.split(","),
      signal: controller.signal,
      eachMessage: (message) =>
        processAsyncConsumerMessage(message, {
          store,
          consumerGroup: group,
          handler: handleDomainEvent,
          workerId: "retention-proof",
        }),
    });
    void consumer.catch(() => controller.abort());
    const published = await processOutboxOnce({
      pool,
      dryRun: false,
      batchSize: 2,
      producer: {
        async send(record) {
          ...
          while (true) {
            ...
            const committed = await admin.fetchOffsets({
              groupId: recovery.transportGroup,
              topics: ["app.tasks"],
            });
            const receipts = await pool.query(
              "SELECT count(*)::int AS n FROM app_async_receipts WHERE task_id=$1",
              [event.eventId],
            );
            if (committed[0].partitions[0].offset === end && receipts.rows[0].n === 1) break;
            assert.ok(
              Date.now() < deadline,
              "Consumer must commit the published event before retention advances",
            );
            await new Promise((resolve) => setTimeout(resolve, 50));
          }
          await admin.deleteTopicRecords({
            topic: "app.tasks",
            partitions: [{ partition: 0, offset: end }],
          });
        },
      },
    });
    assert.equal(
      published.published,
      2,
      "Every claim must use current transport offsets after consumer progress and retention",
    );
  } finally {
    controller.abort();
    try {
...
async function proveRecovery() {
  ...
  for (const key of Object.keys(childEnv)) {
    if (
      /^(DATABASE_|PG|APP_|SESSION_|RATE_LIMIT_|LOGIN_RATE_|REDIS_|KAFKA_|OUTBOX_|ASYNC_|UPLOAD_|OBJECT_STORAGE_|CLICKHOUSE_|BOOTSTRAP_|WORKER_|POSTGRES_)/.test(
        key,
      )
    )
      delete childEnv[key];
    ...
    const done = new Promise((resolve, reject) => {
      ...
      child.once("close", (code, signal) => {
        ...
        if (code === 0) resolve(stdout.trim());
          ...
          reject(
            new Error(
              `${program} ${args[0]} failed (${signal || code}): ${redact(stderr.slice(-2000))}`,
            ),
          );
      });
    });
  ...
  function startRuntime(env) {
    runtime = launch(
      process.execPath,
      ["--import", "tsx", "services/worker/src/index.ts", "async-runtime"],
      env,
    );
  }
  async function checkpoint(label, client, env, eventId) {
    const binding = (
      await client.query("SELECT state,logical_group,transport_group FROM app_kafka_recovery")
    ).rows[0];
    const facts = {
      label,
      eventId,
      outbox: (
        await client.query("SELECT id,status,attempts FROM app_outbox_events WHERE id=$1", [
          eventId,
        ])
      ).rows,
      receipts: (
        await client.query(
          "SELECT task_id,consumer_group,result FROM app_async_receipts WHERE task_id=$1",
          [eventId],
        )
      ).rows,
      tasks: (
        await client.query("SELECT object_id,status FROM app_tasks WHERE object_id=$1", [eventId])
      ).rows,
      dueMessages: JSON.parse(
        await command(
          process.execPath,
          ["--import", "tsx", "scripts/test-async-recovery.mjs", "--inspect-due"],
          env,
        ),
      ),
      offsets: await admin.fetchOffsets({ groupId: prefix, topics: ["app.tasks"] }),
      binding,
      recoveryOffsets: binding
        ? await admin.fetchOffsets({ groupId: binding.transport_group, topics: ["app.tasks"] })
        : undefined,
    };
    summary.checkpoints.push(facts);
    console.log(JSON.stringify(facts));
  }
  const enqueue = (client, eventId) =>
    client.query(
      "INSERT INTO app_outbox_events(id,topic,event_type,trace_id,payload) VALUES($1,'app.tasks','demo.echo',$1,$2::jsonb)",
      [eventId, JSON.stringify({ marker: eventId })],
    );
  const count = async (client, eventId) =>
    (
      await client.query("SELECT count(*)::int AS n FROM app_async_receipts WHERE task_id=$1", [
        eventId,
      ])
    ).rows[0].n;
  async function consumed(client, eventId) {
    ...
    await until("Kafka offset committed to log end", async () => {
      const binding = (
        await client.query("SELECT transport_group FROM app_kafka_recovery WHERE state='ready'")
      ).rows[0];
      const committed = await admin.fetchOffsets({
        groupId: binding?.transport_group || prefix,
        topics: ["app.tasks"],
      });
      const end = await admin.fetchTopicOffsets("app.tasks");
      return end.every(
        (row) =>
          committed[0]?.partitions.find((part) => part.partition === row.partition)?.offset ===
          row.offset,
      );
    });
  }
  ...
  try {
    const postgres = await container(
      "postgres",
      postgresImage,
      { POSTGRES_USER: "app", POSTGRES_PASSWORD: password, POSTGRES_DB: "postgres" },
    ...
    admin = new Kafka({
      clientId: prefix,
      brokers: [`127.0.0.1:${kafkaPort}`],
      logLevel: logLevel.NOTHING,
      retry: { retries: 0 },
      connectionTimeout: 1000,
    }).admin();
    await until("Kafka", async () => {
      await admin.connect();
    ...
    await admin.createTopics({
      topics: [
        { topic: "app.tasks", numPartitions: 1, replicationFactor: 1 },
        { topic: "recovery.extra", numPartitions: 2, replicationFactor: 1 },
      ],
      waitForLeaders: true,
    });
    const env = {
      ...childEnv,
      DATABASE_URL: base + "source",
      APP_TEMPLATE_WORKER_SKIP_ENV_FILES: "1",
      POSTGRES_TOOLS: "docker",
      POSTGRES_TOOL_IMAGE: postgresImage,
      UPLOAD_STORAGE_DRIVER: "local",
      UPLOAD_STORAGE_DIR: path.join(output, "source-uploads"),
      ...
      ASYNC_RUNTIME_TOPICS: "app.tasks,recovery.extra",
      WORKER_HEARTBEAT_PATH: path.join(output, "heartbeat.json"),
    };
    ...
    try {
      const captured = await recoveryTools.captureKafkaCheckpoint(admin, prefix, prefix, [
        "app.tasks",
      ]);
      assert.equal(advancedDuringCapture, true);
      const committed = await recoveryTools.readTransportOffsets(admin, ["app.tasks"], prefix);
      assert.ok(
        BigInt(committed[0].nextOffset) > BigInt(captured.partitions[0].high),
        "Real consumer progress must pass the sampled log end before capture returns",
      );
      assert.ok(
        BigInt(captured.partitions[0].nextOffset) <= BigInt(captured.partitions[0].high),
        "Checkpoint must retain the earlier replayable consumer position",
      );
      summary.concurrentCheckpoint = { captured: captured.partitions, committed };
      console.log(
        "Concurrent checkpoint proof passed: real publication and consumption advance beyond the sampled log end",
      );
    } finally {
      admin.fetchTopicOffsets = fetchTopicOffsets;
    }
    await stopRuntime();
    await source.query(
      "UPDATE app_idempotency_keys SET created_at='2000-01-01', expires_at='2000-01-02' WHERE scope=$1",
      [prefix],
    );
    await source.query("UPDATE app_async_receipts SET created_at='2000-01-01' WHERE task_id=$1", [
      anchor,
    ]);
    ...
    await command(
      process.execPath,
      ["--import", "tsx", "services/worker/src/index.ts", "outbox-once"],
      env,
    );
    await source.query("UPDATE app_outbox_events SET updated_at='2000-01-01' WHERE id=$1", [
      pruned,
    ]);
    await command(
      process.execPath,
      ["--import", "tsx", "scripts/history-prune.mjs", "--days", "1", "--apply"],
      env,
    );
    assert.equal(
      (await source.query("SELECT count(*)::int AS n FROM app_outbox_events WHERE id=$1", [pruned]))
        .rows[0].n,
      0,
    );
    assert.equal(
      (
        await source.query("SELECT response_data FROM app_idempotency_keys WHERE scope=$1", [
          prefix,
        ])
    ...
    const interruptedEnv = {
      ...env,
      DATABASE_URL: base + "interrupted",
      RECOVERY_TEST_BUNDLE: bundle,
    };
    await assert.rejects(
      command(
        process.execPath,
        ["scripts/test-async-recovery.mjs", "--interrupt-restore"],
        interruptedEnv,
      ),
      /SIGKILL/,
    );
    ...
    assert.equal(
      (await interruptedDatabase.query("SELECT count(*)::int AS n FROM app_kafka_recovery")).rows[0]
        .n,
      0,
    );
    await assert.rejects(
      command(
        process.execPath,
        ["--import", "tsx", "services/worker/src/index.ts", "async-runtime", "--iterations", "1"],
        interruptedEnv,
      ),
    ...
    const guardedBefore = (
      await interruptedDatabase.query(
        "SELECT row_to_json(e) snapshot FROM app_outbox_events e WHERE id=$1",
        [guardedEvent],
      )
    ).rows;
    ...
    await command(
      process.execPath,
      ["--import", "tsx", "services/worker/src/index.ts", "outbox-once", "--dry-run"],
      interruptedEnv,
    );
    assert.deepEqual(
      (
    ...
    await assert.rejects(
      command(
        process.execPath,
        ["scripts/app-backup.mjs", "create", "--output", path.join(output, "interrupted-backup")],
        interruptedEnv,
      ),
      /Application restore is incomplete/,
    );
    ...
    await stopRuntime();
    await checkpoint(
      "after snapshot: original consumer completed and committed offset",
      source,
      env,
      eventId,
    );
    ...
    const restoredEnv = {
      ...env,
      DATABASE_URL: base + "restored",
      UPLOAD_STORAGE_DIR: path.join(output, "restored-uploads"),
    };
    await command(
      process.execPath,
      ["scripts/app-backup.mjs", "restore", "--directory", bundle, "--confirm", "--recover-kafka"],
      restoredEnv,
    );
    const restored = await connect(restoredEnv.DATABASE_URL);
    assert.equal(
      (await restored.query("SELECT to_regnamespace('pstack_restore_guard') IS NULL AS absent"))
        .rows[0].absent,
      true,
    );
    await checkpoint(
      "restored snapshot with unchanged Kafka group",
      restored,
      restoredEnv,
      eventId,
    );
    ...
    const completionCounts = (
      await restored.query(
        "SELECT trace_id,count(*)::int AS n FROM app_task_events WHERE event_type='async_task.succeeded' GROUP BY trace_id ORDER BY trace_id",
      )
    ).rows;
    summary.completionCounts = completionCounts;
    assert.deepEqual(
      await admin.fetchOffsets({ groupId: prefix, topics: ["app.tasks"] }),
      originalOffsets,
      "Recovery must not alter the original Kafka group's offsets",
    );
    assert.equal(
      await count(restored, anchor),
      1,
      "Snapshot's completed anchor must retain one receipt",
    );
    assert.equal(
      completionCounts.find((row) => row.trace_id === anchor)?.n,
      1,
      "Recovery must not repeat completed anchor execution",
    );
    assert.equal(
      await count(restored, eventId),
      1,
      "Restored published event must recover its receipt despite Kafka offset already committed after the snapshot",
    );
    assert.equal(
      completionCounts.find((row) => row.trace_id === eventId)?.n,
      1,
      "Recovered event must complete exactly once across restart",
    );
    for (const event of [external, pruned]) {
      assert.equal(
        await count(restored, event),
        1,
        "Recovery must include external producers and messages whose outbox was pruned",
      );
      assert.equal(completionCounts.find((row) => row.trace_id === event)?.n, 1);
    }
    assert.deepEqual(
      (await restored.query("SELECT result FROM app_async_receipts WHERE task_id=$1", [anchor]))
        .rows[0].result,
      {},
      "Replayed tombstone must retain its compacted receipt",
    );
    summary.checks = [
      "exact receipt, external producer, pruned outbox, compacted tombstone, restart, original group unchanged",
      "SIGKILL after database restore before binding leaves durable gate; worker and backup refuse; successful restore clears gate",
    ];
    ...
    await assert.rejects(
      command(
        process.execPath,
        ["scripts/app-backup.mjs", "create", "--output", path.join(output, "narrowed-topics")],
        { ...restoredEnv, ASYNC_RUNTIME_TOPICS: "app.tasks" },
      ),
    ...
    await command(
      process.execPath,
      ["scripts/app-backup.mjs", "create", "--output", secondBundle],
      restoredEnv,
    );
    const secondManifest = JSON.parse(
      await readFile(path.join(secondBundle, "bundle.json"), "utf8"),
    );
    assert.equal(secondManifest.kafkaRecovery.sourceTransportGroup, binding.transport_group);
    assert.equal(
      secondManifest.kafkaRecovery.partitions[0].nextOffset,
      (await admin.fetchOffsets({ groupId: binding.transport_group, topics: ["app.tasks"] }))[0]
        .partitions[0].offset,
    );
    summary.checks.push(
      "three partitions initialized; subsequent backup captures permanent transport and refuses omitted topics",
    );
    ...
    const oneShotBefore = (
      await restored.query("SELECT row_to_json(e) snapshot FROM app_outbox_events e WHERE id=$1", [
        oneShotEvent,
      ])
    ).rows;
    ...
    await restored.query("UPDATE app_kafka_recovery SET state='ready', checkpoint=$1", [
      { ...checkpointData, clusterId: "wrong-cluster" },
    ]);
    await assert.rejects(
      command(
        process.execPath,
    ...
    await command(
      process.execPath,
      ["--import", "tsx", "services/worker/src/index.ts", "outbox-once", "--dry-run"],
      { ...restoredEnv, KAFKA_BROKERS: "127.0.0.1:1" },
    );
    assert.deepEqual(
      (
        await restored.query(
    ...
    assert.equal(
      (await restored.query("SELECT status FROM app_outbox_events WHERE id=$1", [oneShotEvent]))
        .rows[0].status,
      "published",
    );
    startRuntime(restoredEnv);
    ...
    await stopRuntime();
    summary.checks.push(
      "one-shot publisher refuses restore marker, incomplete binding, and ready wrong-cluster checkpoint without claims or Kafka writes; dry-run stays read-only; ready binding publishes and consumes",
    );
    await assert.rejects(
      recoveryTools.validateKafkaHistory(admin, { ...checkpointData, clusterId: "wrong-cluster" }),
      /cluster differs/,
    );
    await admin.createTopics({
      topics: [
    ...
    await assert.rejects(
      recoveryTools.captureKafkaCheckpoint(admin, prefix, prefix, ["compacted"]),
      /cleanup.policy=delete/,
    );
    await assert.rejects(
      recoveryTools.initializeRecoveryTransport(admin, checkpointData, binding.transport_group),
      /already exists/,
    );
    ...
    await assert.rejects(
      ...
      /v2 bundle with a Kafka checkpoint/,
    );
    summary.checks.push(
      "interrupted binding, wrong cluster, compacted topic, existing transport, legacy Kafka recovery rejected",
    );

    const loopFirst = `loop-first-${id}`,
      loopBlocked = `loop-blocked-${id}`;
    await enqueue(restored, loopFirst);
    runtime = launch(
      process.execPath,
      ["--import", "tsx", "services/worker/src/index.ts", "outbox-loop"],
      { ...restoredEnv, OUTBOX_BATCH_SIZE: "1", OUTBOX_POLL_INTERVAL_MS: "1000" },
    );
    await until(
      "outbox-loop first publication",
      async () =>
        (await restored.query("SELECT status FROM app_outbox_events WHERE id=$1", [loopFirst]))
          .rows[0].status === "published",
    );
    process.kill(-runtime.child.pid, "SIGSTOP");
    const loopOffsets = await admin.fetchOffsets({
      groupId: binding.transport_group,
      topics: ["app.tasks", "recovery.extra"],
    });
    ...
    const failedLoop = runtime;
    runtime = undefined;
    process.kill(-failedLoop.child.pid, "SIGCONT");
    const loopDeadline = setTimeout(() => {
      try {
        process.kill(-failedLoop.child.pid, "SIGKILL");
      } catch {}
    }, 30000);
    try {
      await assert.rejects(failedLoop.done, /offset missing or reset/);
    } finally {
    ...
    for (const row of loopOffsets)
      await admin.setOffsets({
        groupId: binding.transport_group,
        topic: row.topic,
        partitions: row.partitions.map(({ partition, offset }) => ({ partition, offset })),
      });
    startRuntime(restoredEnv);
    await consumed(restored, loopBlocked);
    await stopRuntime();
    summary.checks.push(
      "running outbox-loop rejects deleted transport before claiming or sending the next event; restored offsets resume publication",
    );

    for (const phase of ["first", "second"])
      await enqueue(restored, `retention-progress-${phase}-${id}`);
    await command(
      process.execPath,
      ["--import", "tsx", "scripts/test-async-recovery.mjs", "--publish-with-retention"],
      restoredEnv,
    );
    for (const phase of ["first", "second"])
      assert.equal(await count(restored, `retention-progress-${phase}-${id}`), 1);
    summary.checks.push(
      "same batch publishes two events while real consumer commits and retention advances after each send; publisher does not retain stale required offsets",
    );

    const savedOffsets = await admin.fetchOffsets({
      groupId: binding.transport_group,
      topics: ["app.tasks", "recovery.extra"],
    });
    await admin.deleteGroups([binding.transport_group]);
    ...
    await admin.setOffsets({
      groupId: binding.transport_group,
      topic: "app.tasks",
      partitions: savedOffsets[0].partitions.map(({ partition, offset }) => ({
        partition,
        offset,
      })),
    });
    ...
    await admin.setOffsets({
      groupId: binding.transport_group,
      topic: "recovery.extra",
      partitions: savedOffsets[1].partitions.map(({ partition, offset }) => ({
        partition,
        offset,
      })),
    });
    summary.checks.push(
      "deleted transport offsets fail startup without fallback to original group",
    );
    ...
    startRuntime(restoredEnv);
    await until(
      "recovery consumer joined",
      async () =>
        (await admin.describeGroups([binding.transport_group])).groups[0].members.length === 1,
    );
    process.kill(-runtime.child.pid, "SIGSTOP");
    ...
    await admin.deleteTopicRecords({
      topic: "app.tasks",
      partitions: [{ partition: 0, offset: (lostAt + 1n).toString() }],
    });
    ...
    const deadline = setTimeout(() => {
      try {
        process.kill(-stopped.child.pid, "SIGKILL");
      } catch {}
    }, 30_000);
    try {
      await assert.rejects(stopped.done, /history unavailable|offset missing or reset/);
    } finally {
      clearTimeout(deadline);
    }
    assert.equal(
      await count(restored, retainedLater),
      0,
      "Live retention loss must not process a later record after KafkaJS resets offsets",
    );
    await expectWorkerFailure(/history unavailable|offset missing or reset/);
    await assert.rejects(
      recoveryTools.validateKafkaHistory(admin, checkpointData),
      /history unavailable/,
    );
    summary.checks.push(
      "retention loss while consumer is live rejects before later business receipt; expired backup rejected",
    );
    ...
    for (const client of clients)
      await client.end().catch((error) => cleanupErrors.push(error.message));
    // Cleanup remains available after a signal and only addresses this run's random names.
    interrupted = false;
    for (const name of containers.reverse()) {
      try {
        await writeFile(path.join(output, `${name}.log`), await command("docker", ["logs", name]), {
          mode: 0o600,
        });
      } catch {}
      try {

...
export const evidenceSchema = z
  ...
  .superRefine((index, context) => {
    ...
    if (new Set(names).size !== names.length)
      context.addIssue({ code: "custom", message: "Duplicate evidence checks" });
    if (Date.parse(index.finishedAt) < Date.parse(index.startedAt))
      context.addIssue({ code: "custom", message: "Invalid evidence interval" });
    for (const check of index.checks) {
      if (Date.parse(check.finishedAt) < Date.parse(check.startedAt))
        context.addIssue({ code: "custom", message: "Invalid check interval" });
    }
    if (index.status === "passed" && index.checks.some((check) => check.status !== "passed"))
      context.addIssue({ code: "custom", message: "Incomplete evidence cannot pass" });
  });
...
export async function sourceIdentity(root) {
  ...
  return sourceSchema.parse({
    ...
    sourceSha256: sha256(JSON.stringify(Object.fromEntries(hashes))),
  });
}
export async function evidenceReference(root, file) {
  ...
  assert.ok(
    ...
    "Evidence must stay within its root",
  );
...
export async function writeEvidenceIndex(root, run, checks, status) {
  const index = evidenceSchema.parse({
    schemaVersion: 1,
    runId: run.runId,
    source: run.source,
...
export async function verifyEvidence(file, root, expectedSource = null, requirePassed = true) {
  ...
  for (const check of index.checks)
    for (const reference of check.evidence) {
      ...
      assert.deepEqual(
        await evidenceReference(root, path.join(root, reference.path)),
        reference,

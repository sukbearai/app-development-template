...
import { assessCapacityResponse, parseCapacityResponse, summarizeMetrics, summarizeRequests } from './capacity-summary.mjs';
...
export async function runCapacityWorkload({ env, options, signal }) {
  ...
  const writes = [];
  const uploads = [];
  const control = new Client({ connectionString: env.DATABASE_URL, connectionTimeoutMillis: 3000, statement_timeout: 15000, query_timeout: 16000 });
  ...
  async function call(route, init = {}) {
    ...
    const response = await fetch(new URL(route, env.APP_ORIGIN), {
      ...init, redirect: 'error', signal: AbortSignal.any([signal, AbortSignal.timeout(15000)]),
    });
    return readCapacityResponse(response);
  }
  async function sample() {
    const { response, payload } = await call('/api/system/metrics', { headers: { authorization: `Bearer ${env.METRICS_TOKEN}` } });
    ...
    assert.ok(metric.uploads.limit >= 1 && metric.uploads.limit <= 64, 'Metrics upload limit invalid');
    samples.push({ observedAt: metric.observedAt, process: metric.process, databasePool: metric.databasePool, uploads: metric.uploads });
    return metric;
  }
  function upload(filename, contents) {
    const body = new FormData();
    body.set('file', new File([contents], filename, { type: 'text/plain' }));
    return call('/api/uploads', { method: 'POST', headers: { authorization: `Bearer ${token}` }, body });
  }
  async function issue(operation, index) {
    ...
    const role = { id: `${prefix}_${index}`, name: 'Capacity reader', status: 'active', permissionIds: ['admin.read'] };
    const headers = {};
    ...
    return call(login ? '/api/auth/login' : '/api/admin/roles', {
      ...
      headers,
      body: operation === 'read' ? undefined : JSON.stringify(login ? { account: env.UI_FLOW_ADMIN_ACCOUNT, password: env.UI_FLOW_ADMIN_PASSWORD } : role),
    });
  }
    ...
    const workers = Array.from({ length: Math.min(options.concurrency, options.requests) }, async () => {
      while (next < options.requests && !signal.aborted && !samplingError) {
        ...
        try {
          ...
          const assessment = received.diagnostic ? { outcome: 'failed', diagnostic: received.diagnostic } : assessCapacityResponse(operation, response, payload);
          ...
          if (outcome === 'success') {
            ...
            if (operation === 'write') {
              assert.equal(payload.data.id, `${prefix}_${index}`, 'Created role identity mismatch');
              writes.push(payload.data.id);
            }
            if (operation === 'upload') {
              uploads.push({ id: payload.data.id, storageKey: payload.data.storageKey, filename: `${prefix}_${index}.txt` });
            }
          }
        } catch (error) { outcome = 'failed'; diagnostic = capacityFailure(requestStage, error); }
        observations.push({ operation, index, outcome, status, diagnostic, durationMs: performance.now() - start });
      }
    });
  ...
  try {
    ...
    sampler = (async () => {
      ...
      } catch { if (!samplerAbort.signal.aborted && !signal.aborted) samplingError = true; }
    })();
    ...
    stage = 'overload';
    report.overload = await verifyCapacityOverload({ base: env.APP_ORIGIN, token, signal, sample, upload });
    stage = 'persistence';
    const roles = await control.query('SELECT id, name, status FROM app_roles WHERE id = ANY($1::text[])', [writes]);
    ...
    for (const uploadRecord of uploads) {
      signal.throwIfAborted();
      const result = await control.query('SELECT f.storage_key, f.file_name, f.size_bytes, i.state FROM app_file_assets f JOIN app_upload_intents i ON i.storage_key=f.storage_key WHERE f.id=$1', [uploadRecord.id]);
      ...
      assert.ok(location.startsWith(path.resolve(env.UPLOAD_STORAGE_DIR) + path.sep), 'Upload storage path escaped owned directory');
      assert.deepEqual(await readFile(location), Buffer.from(bytes));
      const outbox = await control.query("SELECT count(*)::int AS n FROM app_outbox_events WHERE payload->>'fileId'=$1", [uploadRecord.id]);
      assert.equal(outbox.rows[0].n, 1, 'Uploaded file must have one durable outbox fact');
    }
    report.persistence = { roles: roles.rowCount, files: uploads.length, localFileBytes: uploads.length * options.uploadBytes, committedIntents: uploads.length, uploadOutboxFacts: uploads.length };
    await sample();
    ...
    samplerAbort.abort();
    await sampler;
    if (samplingError) { report.status = 'failed'; report.error = 'capacity_metrics_failed'; }

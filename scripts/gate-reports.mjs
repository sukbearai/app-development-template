...
export async function validateGateReports(gate, files, root, source) {
  ...
  const matches = files.filter(
    (file) =>
      path
        ...
        .split(path.sep)
        .join("/")
        .startsWith(`.verification/${expected.directory}/`) &&
      path.basename(file) === expected.file,
  );
  ...
  if (expected.directory === "app" || expected.directory === "containers")
  ...
  if (expected.mode === "capacity") {
    const capacity = JSON.parse(
      await readFile(path.join(path.dirname(matches[0]), "capacity.json"), "utf8"),
    );

...
async function command(args, database) {
  return run(process.execPath, [path.join(root, "scripts/db-backup.mjs"), ...args], {
    cwd: root,
...
try {
  ...
  for (const name of ["server", "client"]) {
    ...
    await writeFile(
      path.join(certificates, `${name}.ext`),
      name === "server"
  ...
  for (const filename of ["server.crt", "server.key", "ca.crt"]) {
    await writeFile(
      path.join(serverCertificates, filename),
      await readFile(path.join(certificates, filename)),
    );
  }
  await writeFile(
    path.join(serverCertificates, "pg_hba.conf"),
  ...
  containerId = (
    await run(
      ...
      [
        ...
        "--detach",
        "--name",
        `pstack-backup-proof-${suffix}`,
        "--label",
        "pstack.verification=backup",
        "--publish",
        "127.0.0.1::5432",
        "--tmpfs",
        "/var/lib/postgresql/data",
        "--mount",
        `type=bind,source=${serverCertificates},target=/fixtures,readonly`,
        "--env",
        "POSTGRES_USER=app",
        "--env",
        "POSTGRES_PASSWORD",
  ...
  const inspection = JSON.parse(
    await run("docker", ["inspect", containerId], { stdio: ["ignore", "pipe", "inherit"] }),
  )[0];
  ...
  const tls = (
    await control.query("SELECT ssl, client_dn FROM pg_stat_ssl WHERE pid=pg_backend_pid()")
  ).rows[0];
  assert.equal(tls.ssl, true);
  assert.equal(tls.client_dn, "/CN=app");
  for (const database of [source, target, conflict])
    await control.query(`CREATE DATABASE "${database}"`);
  const client = new Client({ connectionString: url(source) });
  await client.connect();
  try {
    await client.query(
      "CREATE TABLE public.backup_probe(id int PRIMARY KEY, value text NOT NULL); INSERT INTO public.backup_probe VALUES (1, 'preserved'); CREATE SCHEMA drizzle; CREATE TABLE drizzle.drizzle_migrations(id serial PRIMARY KEY, hash text NOT NULL, created_at bigint)",
    );
    await client.query("INSERT INTO drizzle.drizzle_migrations(hash,created_at) VALUES ($1,1)", [
      createHash("sha256").update("fixture migration").digest("hex"),
    ]);
  } finally {
    await client.end();
  }
  ...
  try {
    await conflicting.query(
      "CREATE TABLE public.backup_probe(id int PRIMARY KEY, value text NOT NULL); INSERT INTO public.backup_probe VALUES (99, 'concurrent-value')",
    );
    const verified = await verifyBackup({ file });
    await assert.rejects(restoreArchive(verified.file, verified.list, url(conflict)));
    assert.deepEqual((await conflicting.query("SELECT * FROM public.backup_probe")).rows, [
      { id: 99, value: "concurrent-value" },
    ]);
    assert.equal(
      (await conflicting.query("SELECT to_regnamespace('drizzle') IS NULL AS absent")).rows[0]
        .absent,
      true,
    );
  } finally {
  ...
  try {
    assert.equal(
      (await restored.query("SELECT value FROM public.backup_probe WHERE id=1")).rows[0].value,
      "preserved",
    );
    assert.equal(
      (await restored.query("SELECT count(*)::int AS n FROM drizzle.drizzle_migrations")).rows[0].n,
      1,
    );
    await restored.query(
      "BEGIN; ALTER TABLE public.backup_probe ADD COLUMN after_restore boolean NOT NULL DEFAULT false;",
    );
    await restored.query("INSERT INTO drizzle.drizzle_migrations(hash,created_at) VALUES ($1,2)", [
      createHash("sha256").update("next fixture migration").digest("hex"),
    ]);
    await restored.query("COMMIT");
    assert.equal(
      (await restored.query("SELECT after_restore FROM public.backup_probe")).rows[0].after_restore,
      false,
    );
  } finally {
    await restored.end();
  ...
  assert.deepEqual(
    [privateKeyAfter.mode, privateKeyAfter.uid, privateKeyAfter.gid],
    [privateKeyBefore.mode, privateKeyBefore.uid, privateKeyBefore.gid],
  );
  assert.deepEqual((await readdir(output)).sort(), ["manifest.json", "proof.dump"]);
  console.log(
    "TLS backup proof passed: verify-full and client certificate authentication, absolute/relative certificate paths outside archive, quoted/comma/space mounts, unchanged private key permissions",
  );
  console.log(
    "Real backup proof passed: both schemas, ledger, row data, next migration, missing confirmation/checksum, nonempty target refusal, conflicting restore rollback",
  );
} finally {
  try {
    ...
    try {
      if (containerId)
        await run("docker", ["rm", "--force", "--volumes", containerId], { stdio: "ignore" });
    } finally {

...
const historical = new Set(['docs/verification.md', 'docs/production-repair-results.md', 'docs/engineering-adoption-plan.md']);
...
export async function checkDocuments(directory, files) {
  const errors = [];
  let links = 0, references = 0, features = 0;
  ...
  for (const file of files.filter(name => name.endsWith('.md'))) {
    ...
    for (const match of withoutCode.matchAll(/\[[^\]]*\]\(([^\s)]+)(?:\s+"[^"]*")?\)/g)) {
      ...
      links++;
      let resolved;
      try { resolved = decodeURIComponent(target.split(/[?#]/)[0]); } catch { errors.push(`${file}: malformed link ${target}`); continue; }
      const destination = path.resolve(path.dirname(path.join(directory, file)), resolved);
      if (!destination.startsWith(`${path.resolve(directory)}/`) || !await exists(destination)) errors.push(`${file}: broken local link ${target}`);
    }
    ...
    for (const [, reference] of text.matchAll(/`([^`\n]+)`/g)) {
      ...
      references++;
      if (!await exists(path.join(directory, reference))) errors.push(`${file}: missing example/source path ${reference}`);
    }
    if (file === '.agents/skills/verify-pstack-x/features/README.md') {
      for (const line of text.split('\n').filter(line => /^\| (?!Feature|---)/.test(line))) {
        ...
        for (const index of [1, 3]) {
          const paths = (cells[index] ?? '').split(',').map(item => item.trim().replaceAll('`', ''));
          if (!paths.length || paths.some(item => !sourcePath.test(item))) errors.push(`${file}: ${cells[0]} requires explicit ${index === 1 ? 'source' : 'verification'} paths`);
          for (const item of paths.filter(item => sourcePath.test(item))) {
            if (!await exists(path.join(directory, item))) errors.push(`${file}: ${cells[0]} missing ${item}`);
          }
        }
      }
    }
  }
  const recipe = JSON.parse(await readFile(path.join(directory, 'docs/startup-steps.json'), 'utf8'));
  ...
  for (const args of Object.values(recipe)) {
    ...
    if (args[0] !== 'install' && !manifest.scripts[args[0]]) errors.push(`package.json: missing startup script ${args[0]}`);
  }
...
export async function docsMain(args = process.argv.slice(2)) {
  ...
  let data = null, status = 'passed', errorCode = null;
  try {
    if (args.some(arg => arg !== '--json') || args.length > 1) throw Object.assign(new Error('Only --json is supported.'), { code: 'INVALID_ARGUMENT' });
    const files = [...new Set(execFileSync('git', ['ls-files', '-z', '--cached', '--others', '--exclude-standard'], { cwd: root }).toString().split('\0'))]
      .filter(file => file === 'README.md' || file.startsWith('docs/') || /^(?:packages|services)\/[^/]+\/README\.md$/.test(file) || file.startsWith('.agents/skills/verify-pstack-x/'));
    data = await checkDocuments(root, files);
    if (data.errors.length) { status = 'failed'; errorCode = 'DOCS_STALE'; }
  } catch (error) {
  ...
  if (!args.includes('--json') && data.errors.length) process.stderr.write(`${data.errors.join('\n')}\n`);
  printCommandResult(result, args.includes('--json'));
}
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) await docsMain();

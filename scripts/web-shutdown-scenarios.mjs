...
export async function verifyWebShutdown({ launch, env, output }) {
  ...
  async function stopWhileBlocked(server) {
    ...
    const chunks = on(server.stderr, "data", {
  ...
  try {
    ...
    server.kill("SIGINT");
    const admission = await fetch(`${base}/api/hello`, { signal: AbortSignal.timeout(500) }).then(
      (response) => response.status,
      () => "closed",
    );
    assert.ok(
      admission === "closed" || admission === 503,
      `New admission during drain: ${admission}`,
    );
    request.client.write(
      request.body.slice(5) +
    ...
    checks.push(
      "slow request body completes; repeated signals do not cut drain short; new connections and pipelined requests are rejected",
    );
    ...
    const recorded = fetch(`${base}/api/telemetry`, {
      method: "POST",
      headers: { "content-type": "application/json", "x-trace-id": trace },
      body: JSON.stringify({ event: "shutdown.proof" }),
    ...
    assert.equal(
      (
        await control.query(
          "SELECT count(*)::int AS n FROM app_telemetry_events WHERE trace_id=$1",
          [trace],
        )
      ).rows[0].n,
      1,
    ...
    assert.equal(
      (
        await control.query("SELECT count(*)::int AS n FROM app_outbox_events WHERE trace_id=$1", [
          trace,
        ])
      ).rows[0].n,
      1,
    ...
    checks.push(
      "SIGTERM waits for the actual PostgreSQL transaction and returns the complete 201 response",
    );
    ...
    const uploaded = fetch(`${base}/api/uploads`, {
      method: "POST",
    ...
    const files = await control.query(
      "SELECT id, storage_key FROM app_file_assets WHERE file_name=$1",
      [filename],
    );
    assert.equal(files.rows.length, 1);
    const file = files.rows[0];
    assert.equal(
      await readFile(path.join(env.UPLOAD_STORAGE_DIR, file.storage_key), "utf8"),
      bytes,
    );
    assert.equal(
      (
        await control.query("SELECT state FROM app_upload_intents WHERE storage_key=$1", [
          file.storage_key,
        ])
      ).rows[0].state,
      "committed",
    ...
    checks.push(
      "client disconnect cannot release the upload business work; local bytes, file row, committed intent and outbox survive SIGTERM",
    );
    ...
    assert.ok(
      elapsed >= 1100 && elapsed < 1900,
      "Repeated signal must not extend the single deadline",
    );
    await hung.closed;
    checks.push(
      "hung request exits 1 within the configured absolute deadline; repeated SIGTERM does not reset it",
    );
    server = await launch();
    ...
    await writeFile(
      path.join(output, "web-shutdown.json"),

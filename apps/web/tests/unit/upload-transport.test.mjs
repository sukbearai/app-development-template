...
function request(address, agent, method) {
  return new Promise((resolve, reject) => {
    const outgoing = http.request(address + (method === "POST" ? "/api/uploads" : "/api/hello"), {
      method, agent, headers: method === "POST" ? { "content-length": 262144, "content-type": "multipart/form-data; boundary=fixture" } : {},
    }, (incoming) => {
      const chunks = [];
...
for (const [status, code] of [[401, "UNAUTHORIZED"], [413, "UPLOAD_TOO_LARGE"], [503, "UPLOAD_BUSY"], [200, null]]) {
  ...
  const server = http.createServer(async (incoming, outgoing) => {
    ...
    const response = await withAccessLog(webRequest, "trace-transport", async () => {
      if (incoming.method === "POST") {
        ...
        return Response.json({ traceId: "trace-transport", data: {
          id: "file-fixture", fileName: "fixture.txt", mimeType: "text/plain", sizeBytes: bytes.byteLength,
          storageKey: "upload-fixture", uploadedBy: "fixture-user", uploadedAt: new Date().toISOString(),
        } });
      }

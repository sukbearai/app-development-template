...
async function launch(t, scenario, timeoutMs = 1500) {
  ...
  await copyFile(new URL('../fixtures/web-startup.mjs', import.meta.url), path.join(directory, 'dist/server/index.js'));
  const child = spawn(process.execPath, [fileURLToPath(new URL('../../scripts/start.mjs', import.meta.url)), '--hostname', '127.0.0.1', '--port', String(port)], {
    cwd: directory,
    env: { ...process.env, NODE_ENV: 'production', WEB_SHUTDOWN_TIMEOUT_MS: String(timeoutMs), PSTACK_STARTUP_SCENARIO: scenario },
  ...
  async function waitFor(event) {
    ...
    await Promise.race([waiter.promise, exited.then(result => {
      throw new Error(`Exited before ${event}: ${JSON.stringify(result)}\n${log}`);
    })]);
  }
  await waitFor('startup');
  return {
    child, exited, waitFor, port,
    log: () => log,
    events: () => readFile(path.join(directory, 'events.txt'), 'utf8').catch(error => {
...
test('SIGTERM during real vinext import joins late resources and rejects new admission', { timeout: 10000 }, async t => {
  ...
  const requests = (async () => {
    while (probing) {
      await fetch(`http://127.0.0.1:${fixture.port}/`, { signal: AbortSignal.timeout(100) }).then(response => {
  ...
  assert.match(fixture.log(), /Production server running at/, 'vinext never returned its late listening server');
  assert.equal(await fixture.events(), 'work-rejected\ncleanup-first\ncleanup-late\n');
});

test('startup rejection preserves failure through registered cleanup and signal cancellation', { timeout: 10000 }, async t => {
  ...
  const result = fixture.log().trim().split('\n').map(line => JSON.parse(line)).at(-1);
...
test('startup and cleanup diagnostics never serialize exception secrets', { timeout: 10000 }, async t => {
...
for (const scenario of ['startup-hang', 'cleanup-hang']) {
  test(`${scenario} shares the first signal deadline across repeated signals`, { timeout: 10000 }, async t => {
    ...
    assert.equal(await fixture.events(), scenario === 'startup-hang' ? '' : 'work-rejected\ncleanup-first\ncleanup-late\n');
  });
}

test('one failed disposer still attempts and awaits later cleanup exactly once', { timeout: 10000 }, async t => {
...
test('closed stderr cannot bypass startup joining or resource cleanup', { timeout: 10000 }, async t => {
  const fixture = await launch(t, 'delayed');

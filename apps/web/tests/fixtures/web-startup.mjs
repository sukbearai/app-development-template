...
const released = new Promise(resolve => process.on('message', message => {
...
lifecycle.registerCleanup(async () => {
  ...
  if (scenario === 'secret-failure') throw new Error('postgres://user:secret-sentinel@host/database');
...
if (lifecycle.draining) {
  try { await lifecycle.trackWork(() => record('unexpected-work')); }
  catch { record('work-rejected'); }
}
export default () => {

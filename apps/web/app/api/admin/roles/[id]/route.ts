...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return withAccessLog(request, traceId, async () => {
    ...
    const body = parseInput(updateRoleRequestSchema, await readJson(request));
    return ok(await updateManagedRole(params.id, body, authToken(request), traceId), traceId);
  });
}

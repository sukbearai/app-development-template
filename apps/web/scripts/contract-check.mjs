...
async function listRouteFiles(directory, prefix = "app/api") {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) files.push(...await listRouteFiles(new URL(`${entry.name}/`, directory), relative));
...
function exportedMethods(source, filename) {
  ...
  for (const statement of parsed.statements) {
    ...
    if (ts.isVariableStatement(statement)) {
      exports.push(...statement.declarationList.declarations.flatMap((declaration) =>
...
assert.equal(new Set(apiRoutes.map((operation) => operation.operationId)).size, apiRoutes.length, "Duplicate operation ID");
assert.equal(new Set(apiRoutes.map((operation) => `${operation.method} ${operation.path}`)).size, apiRoutes.length, "Duplicate HTTP operation");
const explicit = routeFiles.filter((file) => file !== "app/api/[...segments]/route.ts");
assert.deepEqual([...new Set(apiRoutes.map((operation) => operation.routeFile))].sort(), explicit,
...
for (const file of routeFiles) {
  ...
  assert.deepEqual(exportedMethods(source, file), operations.map((operation) => operation.method).sort(), `${file}: method exports differ from registry`);
  for (const operation of operations) {
    assert.equal(file, `app${operation.path.replace(/\{([^}]+)\}/g, "[$1]")}/route.ts`, "Route path does not match its file");
    if (operation.request) {
      assert.deepEqual(document.components.schemas[operation.request.name], jsonSchema(operation.request.schema, "input"),
    ...
    for (const [status, response] of Object.entries(operation.responses)) {
      ...
      assert.deepEqual(definition, jsonSchema(response.schema, "output"),
        `${operation.operationId} ${status}: response schema name collides with another definition`);
      assert.equal(definition.type, "object", `${operation.operationId} ${status}: response must have a concrete shape`);
      if (Number(status) < 400 && operation.path !== "/api/hello") {
        assert.ok(definition.required.includes("data"));
        assert.ok(definition.properties.data.type || definition.properties.data.$ref || definition.properties.data.anyOf,
          `${operation.operationId} ${status}: success data must be typed`);
      }
      assert.equal(response.schema.safeParse({}).success, false, `${operation.operationId} ${status}: empty response accepted`);
    }
  }
}
console.log(`contract check ok (${apiRoutes.length} operations; route methods, generated inputs and outputs)`);

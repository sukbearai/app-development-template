...
export async function runWebProcess({ lifecycle, timeoutMs, start }) {
  ...
  function requestStop() {
    ...
    deadline = setTimeout(() => {
      try { log('error', 'Web shutdown deadline exceeded'); }
      finally { process.exit(1); }
    }, timeoutMs);
    stopped.resolve();
    if (server) closeServer();
    try { log('info', 'Web shutdown started'); }
    catch (error) { failures.push(error); }
  }
  function fail(error) {
    failures.push(error);
  ...
  await startup;
  await closing;
  try { await lifecycle.drain(); }
  catch (error) { failures.push(error); }
  clearTimeout(deadline);
  for (const signal of signals) process.off(signal, requestStop);
  try {
    log(failures.length ? 'error' : 'info', failures.length ? 'Web shutdown cleanup failed' : 'Web shutdown completed', failures.length);

...
const login = await call('/api/auth/login', { method: 'POST', body: { account, password }, origin: base });
...
assert.equal((await call('/api/auth/logout', { method: 'POST', token: forged, body: {} })).status, 200);
...
assert.equal((await call('/api/admin/roles', { method: 'POST', cookie, origin: 'https://evil.invalid', body: {} })).status, 403);
checks.push('cross-origin cookie write rejected');
const suffix = String(Date.now());
const role = { id: `role_smoke_${suffix}`, name: 'Smoke reader', permissionIds: ['admin.read'], status: 'active' };
assert.equal((await call('/api/admin/roles', { method: 'POST', token, body: role })).status, 201);
const user = { account: `smoke_${suffix}`, displayName: 'Smoke user', password: 'Smoke-Test-Password-42!', roleIds: [role.id], status: 'enabled' };
...
const reader = await call('/api/auth/login', { method: 'POST', body: { account: user.account, password: user.password } });
assert.equal(reader.status, 200);
const readerToken = reader.body.data.token;
assert.equal((await call('/api/admin/users', { token: readerToken })).status, 200);
assert.equal((await call('/api/admin/roles', { method: 'POST', token: readerToken, body: role })).status, 403);
assert.equal((await call(`/api/admin/roles/${role.id}`, { method: 'PATCH', token, body: { status: 'inactive' } })).status, 200);
assert.equal((await call('/api/admin/users', { token: readerToken })).status, 403);
checks.push('read-only role cannot write; inactive role immediately loses access; duplicate user conflicts');
assert.equal((await call(`/api/admin/users/${userId}`, { method: 'PATCH', token, body: { status: 'disabled' } })).status, 200);
assert.equal((await call(`/api/admin/users/${userId}`, { method: 'PATCH', token, body: { status: 'enabled' } })).status, 200);
...
assert.equal((await call('/api/telemetry', { method: 'POST', body: { event: 'smoke.completed', route: '/' } })).status, 201);

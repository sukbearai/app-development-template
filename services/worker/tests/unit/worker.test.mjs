  ...
  for (const locale of ["en_US.UTF-8", "sv_SE.UTF-8", "tr_TR.UTF-8"]) {
    const child = spawnSync(
      ...
      [
        ...
        "--input-type=module",
        "--eval",
        `import { payloadHash } from ${JSON.stringify(new URL("../../src/async-task.ts", import.meta.url).href)}; process.stdout.write(JSON.stringify({ hash: payloadHash(${JSON.stringify(task)}), locale: Intl.DateTimeFormat().resolvedOptions().locale }));`,
      ],

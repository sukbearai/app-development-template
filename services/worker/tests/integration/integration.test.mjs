...
import { claimEvents, markPublished, processOutboxOnce } from "../../src/outbox.ts";
import { createPostgresAsyncTaskStore } from "../../src/async-task-store.ts";
import {
...
import { processAsyncConsumerMessage, runKafkaConsumer } from "../../src/async-consumer.ts";
...
if (!process.env.WORKER_TEST_DATABASE_URL)
  throw new Error("WORKER_TEST_DATABASE_URL must identify an isolated empty test database");
const pool = new Pool({
...
before(async () => {
  ...
  assert.equal(count.rows[0].n, 0, "integration test requires an empty database");
  const directory =
    process.env.WORKER_TEST_MIGRATIONS ??
    new URL("../../../../packages/database/migrations/template/", import.meta.url).pathname;
...
test("dry-run observes without changing any persisted outbox bytes", async () => {
  ...
  const before = await pool.query("SELECT row_to_json(e) snapshot FROM app_outbox_events e");
  const result = await processOutboxOnce({ pool, dryRun: true });
  assert.equal(result.inspected, 1);
  assert.equal(result.claimed, 0);
  assert.equal(result.published, 0);
  assert.deepEqual(
    (await pool.query("SELECT row_to_json(e) snapshot FROM app_outbox_events e")).rows,
    before.rows,
  );
});
...
test("outbox publishing refuses incomplete restores while dry-run stays read-only", async () => {
  ...
  await pool.query(
    "INSERT INTO app_outbox_events(id,topic,event_type,trace_id,payload,created_at) VALUES($1,'app.tasks','demo.echo','restore-guard','{}','1970-01-01')",
    [id],
  );
  let sends = 0;
  const producer = {
  ...
  const snapshot = async () =>
    (await pool.query("SELECT row_to_json(e) snapshot FROM app_outbox_events e WHERE id=$1", [id]))
      .rows;
  try {
    for (const state of ["guard", "restoring"]) {
      if (state === "guard") await pool.query("CREATE SCHEMA pstack_restore_guard");
        ...
        await pool.query(
          "INSERT INTO app_kafka_recovery(state,logical_group,transport_group,checkpoint) VALUES('restoring',$1,'data-only-recovery-disabled','{}')",
          [group],
        );
      ...
      await assert.rejects(
        processOutboxOnce({ pool, producer, dryRun: false, batchSize: 1 }),
        /restore|recovery/i,
      );
...
test("concurrent identical delivery executes one transactional domain effect and one receipt", async () => {
  ...
  const key = JSON.stringify([group, parseAsyncTaskMessage(input, group).idempotencyKey]);
  assert.equal(
    (
      await pool.query("SELECT count(*)::int n FROM app_async_receipts WHERE idempotency_key=$1", [
        key,
      ])
    ).rows[0].n,
    1,
  ...
  assert.equal(
    (await pool.query("SELECT status FROM app_idempotency_keys WHERE key=$1", [key])).rows[0]
      .status,
    "succeeded",
  );
});
...
test("Unicode payload survives JSONB durable retry and later duplicate delivery", async () => {
  ...
  assert.equal(
    (await processAsyncConsumerMessage(retry, { ...settings, store: recreated })).status,
    "succeeded",
  );
  assert.equal((await processAsyncConsumerMessage(input, settings)).status, "skipped_duplicate");
  assert.equal(
    (await processAsyncConsumerMessage(message(id, { "\u00e9": 2, "e\u0301": 1 }), settings))
      .errorCode,
...
test("legacy failed Unicode JSONB task recovers without rewriting historical receipt identity", async () => {
  ...
  const recreated = createPostgresAsyncTaskStore({ pool });
  const retry = (await recreated.dueMessages(group, 1000)).find(
    (item) => JSON.parse(item.value).eventId === JSON.parse(input.value).eventId,
  );
  assert.ok(retry);
  ...
  assert.equal((await processAsyncConsumerMessage(input, settings)).status, "skipped_duplicate");
  const row = (
    await pool.query("SELECT request_hash FROM app_idempotency_keys WHERE key=$1", [key])
  ).rows[0];
  const receipt = (
    await pool.query("SELECT payload_hash FROM app_async_receipts WHERE idempotency_key=$1", [key])
  ).rows[0];
...
test("legacy terminal payload proves equality and detects contradictions even when the legacy hash matches", async () => {
  ...
  const settings = {
    ...options,
    handler: async () => assert.fail("terminal task must not execute"),
  };
  const reordered = message(id, { "\u00e9": 1, "e\u0301": 2 });
  assert.notEqual(legacyHash(parseAsyncTaskMessage(reordered, group)), hash);
  assert.equal(
    (await processAsyncConsumerMessage(reordered, settings)).status,
    "skipped_duplicate",
  );
  await pool.query(
    "UPDATE app_idempotency_keys SET response_data=jsonb_set(response_data,'{payload}', '{\"changed\":true}') WHERE key=$1",
    [key],
  );
  assert.equal(
    (await processAsyncConsumerMessage(input, settings)).errorCode,
    "IDEMPOTENCY_CONFLICT",
  );
  assert.equal(
    (await pool.query("SELECT request_hash FROM app_idempotency_keys WHERE key=$1", [key])).rows[0]
      .request_hash,
    hash,
  );
});
...
test("compacted legacy identities only accept exact old hash matches", async () => {
  ...
  const settings = {
    ...options,
    handler: async () => assert.fail("compacted task must not execute"),
  };
  assert.equal((await processAsyncConsumerMessage(input, settings)).status, "skipped_duplicate");
...
test("unsupported hashes and incomplete nonterminal stored tasks cannot execute", async () => {
  ...
  for (const fixture of [
    ...
    await pool.query("DELETE FROM app_idempotency_keys WHERE key=$1", [
      JSON.stringify([group, parseAsyncTaskMessage(input, group).idempotencyKey]),
    ]);
  }
  const input = message();
  await seedIdentity(input, {
    compact: true,
    hash: payloadHash(parseAsyncTaskMessage(input, group)),
  });
  assert.equal(
    (await processAsyncConsumerMessage(input, settings)).errorCode,
    "IDEMPOTENCY_UNVERIFIABLE",
...
test("stored envelope parsing preserves historical dates, raw identifiers and extension fields", async () => {
  ...
  const saved = (
    await pool.query("SELECT response_data FROM app_idempotency_keys WHERE key=$1", [key])
...
test("stored envelope parsing rejects missing payload and distinguishes invalid source metadata", async () => {
  for (const [transform, diagnostic] of [
    ...
    [
      (task) => ({ ...task, source: { ...task.source, eventId: "other" } }),
      "Stored task source or metadata is invalid",
    ],
  ]) {
    const input = message();
    const task = parseAsyncTaskMessage(input, group);
    ...
    await assert.rejects(
      store.claim(task, "invalid-envelope-worker"),
      (error) => error instanceof PayloadUnverifiableError && error.message === diagnostic,
    );
...
test("payload key order is canonical but changed values are quarantined without modifying success", async () => {
  ...
  assert.equal(
    (await processAsyncConsumerMessage(message(id, { b: 2, a: 1 }), options)).status,
    "skipped_duplicate",
  );
...
test("receipt insert failure rolls back domain write and success event before durable failure", async () => {
  ...
  assert.match(result.errorMessage, /duplicate key value violates unique constraint/);
  assert.equal(
    (await pool.query("SELECT count(*)::int n FROM app_telemetry_events WHERE id=$1", [id])).rows[0]
      .n,
    0,
  );
  assert.equal(
...
test("offset failure preserves success, domain projection, and completed receipt", async () => {
  ...
  assert.equal((await processAsyncConsumerMessage(input, options)).status, "skipped_duplicate");
...
test("retry survives store recreation, defers until due, and dead letter replay is durable", async () => {
  ...
  assert.equal((await processAsyncConsumerMessage(input, broken)).status, "failed");
  const fresh = createPostgresAsyncTaskStore({ pool });
  assert.equal(
    (await processAsyncConsumerMessage(input, { ...broken, store: fresh })).status,
    "deferred_retry",
  );
  await sleep(120);
  ...
  assert.equal(await fresh.replay(group, parseAsyncTaskMessage(input, group).idempotencyKey), true);
  const due = await fresh.dueMessages(group);
  assert.ok(due.some((item) => JSON.parse(item.value).eventId === JSON.parse(input.value).eventId));
  assert.equal(
    (await processAsyncConsumerMessage(input, { ...options, store: fresh })).status,
    "succeeded",
  );
});
...
test("stale task owner cannot execute or fail after another owner recovers", async () => {
  ...
  await assert.rejects(() => short.execute(first.task, handleDomainEvent), StaleLeaseError);
  await assert.rejects(() => short.fail({ ...first.task, status: "failed" }), StaleLeaseError);
...
test("malformed and null messages persist quarantine before acknowledgement", async () => {
  ...
  assert.equal(
    (await pool.query("SELECT count(*)::int n FROM app_message_quarantine WHERE topic='poison'"))
      .rows[0].n,
    6,
  );
});
...
test("oversized indexed identifiers are quarantined before PostgreSQL claim and acknowledged", async () => {
  for (const [index, field] of [
    ...
    const result = await processAsyncConsumerMessage(input, {
      ...
      handler: async () => {
        assert.fail("oversized message reached handler");
      },
    });
    assert.equal(result.status, "quarantined");
...
test("index budget boundary succeeds and legacy serialized terminal keys still deduplicate", async () => {
  ...
  const idempotencyKey = randomBytes(1000)
    .toString("hex")
    .slice(0, 2000 - overhead);
  const input = message(
    randomUUID(),
    {},
    {
  ...
  const duplicate = await processAsyncConsumerMessage(legacyInput, {
    ...options,
    handler: async () => {
      assert.fail("legacy successful key executed again");
    },
  });
...
async function recoverySnapshot(key) {
  return (
    await pool.query(
      "SELECT to_jsonb(task) AS snapshot FROM app_idempotency_keys task WHERE key=$1",
      [key],
    )
  ).rows[0].snapshot;
}
...
test("malformed recovery records are durably isolated without changing their original evidence", async () => {
  ...
  const transforms = [
    ...
    (task) => ({
      ...task,
      source: { ...task.source, offset: { ...task.source.offset, consumerGroup: "another-group" } },
    }),
  ];
  const invalid = [];
  ...
  await pool.query("UPDATE app_idempotency_keys SET response_data=NULL WHERE key=$1", [
    invalid[0].key,
  ]);
  invalid[0].snapshot = await recoverySnapshot(invalid[0].key);
  await pool.query(
    "UPDATE app_idempotency_keys SET status='processing',lease_until=NULL WHERE key=$1",
    [invalid[1].key],
  );
  ...
  assert.equal(
    (await processAsyncConsumerMessage(due[0], { ...options, consumerGroup })).status,
    "succeeded",
  );
  for (const row of invalid) {
    const isolated = (
      await pool.query("SELECT * FROM app_async_recovery_quarantine WHERE idempotency_key=$1", [
        row.key,
      ])
    ).rows;
  ...
  assert.equal(
    (
      await pool.query(
        "SELECT count(*)::int n FROM app_message_quarantine WHERE consumer_group=$1",
        [consumerGroup],
      )
    ).rows[0].n,
    0,
...
test("recovery isolation failure preserves the cursor and retries before later work", async () => {
  ...
  try {
    ...
    await pool.query(
      "DROP TRIGGER reject_recovery_test ON app_async_recovery_quarantine; DROP FUNCTION reject_recovery_test()",
    );
  }
  const due = await fresh.dueMessages(consumerGroup, 1);
  assert.equal(due.length, 1);
...
test("bounded recovery scanning reaches due work after a larger future backlog", async () => {
  ...
  for (let index = 0; index < 205; index++)
    await seedRecoveryRow(consumerGroup, index, (task) => ({
      ...task,
  ...
  assert.deepEqual(
    await fresh.dueMessages(consumerGroup, 1),
    due,
    "unprocessed due message stays visible",
  );
  assert.equal(
    (await processAsyncConsumerMessage(due[0], { ...options, consumerGroup })).status,
    "succeeded",
  ...
  for (let poll = 0; poll < 4; poll++)
    assert.deepEqual(await restarted.dueMessages(consumerGroup, 1), []);
  assert.equal(
    (
      await pool.query(
        "SELECT count(*)::int n FROM app_idempotency_keys WHERE scope=$1 AND status='failed'",
        [consumerGroup],
      )
    ).rows[0].n,
    205,
  ...
  for (const activeLease of [false, true]) {
    ...
    try {
      await pool.query(
        ...
        [
          bad.key,
          JSON.stringify(bad.task),
          activeLease ? "processing" : "failed",
          activeLease ? "2999-01-01" : null,
          activeLease ? "current-owner" : null,
        ],
      );
    } finally {
      resume();
    ...
    assert.equal(
      (
        await pool.query(
          "SELECT count(*)::int n FROM app_async_recovery_quarantine WHERE idempotency_key=$1",
          [bad.key],
        )
      ).rows[0].n,
      0,
...
test("concurrent recovery isolation stays idempotent and its marker blocks corrected-row claim and replay", async () => {
  ...
  await pool.query("UPDATE app_idempotency_keys SET response_data=$2::jsonb WHERE key=$1", [
    bad.key,
    JSON.stringify(bad.task),
  ]);
  const corrected = await recoverySnapshot(bad.key);
...
test("recovery keyset scan advances across equal creation timestamps", async () => {
  ...
  await pool.query("UPDATE app_idempotency_keys SET created_at='2020-01-01' WHERE scope=$1", [
    consumerGroup,
  ]);
  const fresh = createPostgresAsyncTaskStore({ pool });
  assert.deepEqual(await fresh.dueMessages(consumerGroup, 1), []);
...
test("recovery scan wraps despite new future records arriving beyond its cycle boundary", async () => {
  ...
  for (let index = 0; index < 102; index++) {
    const row = await seedRecoveryRow(consumerGroup, index, (task) => ({
      ...task,
      nextRetryAt: "2999-01-01T00:00:00Z",
    }));
    first ??= row;
  ...
  assert.deepEqual(await fresh.dueMessages(consumerGroup, 1), []);
  await pool.query(
    "UPDATE app_idempotency_keys SET response_data=response_data-'nextRetryAt' WHERE key=$1",
    [first.key],
  );
...
test("quarantined legacy oversized retries cannot starve later valid durable work", async () => {
  ...
  for (let index = 0; index < 11; index++) {
    ...
    await pool.query(
      ...
      [
        JSON.stringify([legacyGroup, task.idempotencyKey]),
        legacyGroup,
        payloadHash(task),
        JSON.stringify(task),
        index,
  ...
  try {
    await assert.rejects(
      processAsyncConsumerMessage(first[0], {
        ...options,
    ...
    await pool.query(
      "DROP TRIGGER reject_quarantine_test ON app_message_quarantine; DROP FUNCTION reject_quarantine_test()",
    );
  }
  ...
  assert.equal(
    next.length,
    1,
    "already quarantined legacy entries must leave room for later valid work",
  );
  const result = await processAsyncConsumerMessage(next[0], {
    ...options,
    consumerGroup: legacyGroup,
  });
  assert.equal(result.status, "succeeded");
...
test("unverifiable durable tasks leave the retry window only after quarantine commits", async () => {
  ...
  for (let index = 0; index < 11; index++) {
    ...
    await pool.query(
      ...
      [
        key,
        consumerGroup,
        index < 10 ? `v3:${"a".repeat(64)}` : payloadHash(task),
        JSON.stringify(task),
        index,
      ],
    );
  ...
  const preserved = await pool.query(
    "SELECT request_hash,response_data FROM app_idempotency_keys WHERE key=ANY($1::text[])",
    [keys.slice(0, 10)],
  );
...
test("conflict quarantine and other Kafka sources do not hide a recoverable task", async () => {
  ...
  await pool.query(
    ...
    [
      JSON.stringify([recoveryGroup, task.idempotencyKey]),
      recoveryGroup,
      payloadHash(task),
      JSON.stringify(task),
    ],
  ...
  assert.equal(due.length, 1);
  assert.equal(
    (await processAsyncConsumerMessage(due[0], { ...options, consumerGroup: recoveryGroup }))
      .status,
    "succeeded",
  );
});
...
test("real Kafka outbox delivery retries exact offset then advances and quarantines poison", async () => {
  if (!process.env.WORKER_TEST_KAFKA_BROKERS)
    throw new Error("WORKER_TEST_KAFKA_BROKERS is required for complete middleware verification");
  ...
  try {
    ...
    assert.equal((await processOutboxOnce({ pool, producer, dryRun: false })).published, 1);
    ...
    await producer.send({
      ...
      messages: [
        ...
        {
          value: message("long-key", {}, { idempotencyKey: randomBytes(4000).toString("hex") })
            .value,
        },
      ...
      eachMessage: async (input) => {
        ...
        return processAsyncConsumerMessage(input, {
          ...
          handler: async (task) => {
            ...
            if (task.sourceEventId === id && ++calls === 1) throw new Error("retry once");
...
test("SIGKILL after broker acceptance recovers publisher lease and duplicate yields one receipt", async () => {
  ...
  const child = fork(new URL("../fixtures/crash-publisher.mjs", import.meta.url), [], {
  ...
  try {
    ...
    assert.equal(executions, 1);
    assert.equal(
      (await pool.query("SELECT lease_generation,status FROM app_outbox_events WHERE id=$1", [id]))
...
test("runtime SIGTERM stops claims, closes Kafka and pool, and marks heartbeat stopped", async () => {
  ...
  const heartbeat = new URL(`./.heartbeat-${randomUUID()}.json`, import.meta.url).pathname;
  ...
  try {
    ...
    assert.ok(output.includes('"iteration":1'), `runtime did not start: ${stderr}`);
    ...
    try {
      await producer.send({
        ...
        messages: [
          {
            value: message(eventId, {}, { eventType: "unsupported.test" }).value,
          },
        ],
      });
    } finally {
    ...
    const exit = new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("SIGTERM drain timeout")), 15000);
    ...
    assert.equal(JSON.parse(await readFile(heartbeat, "utf8")).state, "stopped");
  } finally {
    child.kill("SIGKILL");
...
test(
  "runtime SIGTERM drains an in-flight receipt transaction and restart skips duplicate execution",
  { timeout: 90000 },
  async (t) => {
    const { spawn } = await import("node:child_process");
    const { rm } = await import("node:fs/promises");
    ...
    function startRuntime() {
      ...
      child.exit = new Promise((resolve) =>
      ...
      processHandle.stdout.on("data", (data) => {
        child.output = (child.output + data).slice(-16384);
      });
      processHandle.stderr.on("data", (data) => {
        child.stderr = (child.stderr + data).slice(-16384);
      });
      children.push(child);
      return child;
    }
    ...
    async function snapshot() {
      return {
        ...
        receipts: (
          await pool.query("SELECT * FROM app_async_receipts WHERE idempotency_key=$1", [key])
        ).rows,
        events: (
          await pool.query(
            "SELECT * FROM app_task_events WHERE task_id=$1 ORDER BY created_at,id",
            [taskId],
          )
        ).rows,
      };
    ...
    try {
      ...
      await waitFor(
        "worker did not block on the owned receipt-table lock",
        async () => (await blockedReceipt()).rowCount === 1,
      );
      const claimed = (
        await pool.query("SELECT status,locked_by FROM app_idempotency_keys WHERE key=$1", [key])
      ).rows[0];
      ...
      assert.equal(
        (await blockedReceipt()).rowCount,
        1,
        "receipt transaction remains owned while draining",
      );
      ...
      assert.deepEqual(
        completed.events.map((event) => event.status),
        ["running", "succeeded"],
      );

      const restarted = startRuntime();
      await waitFor("restarted runtime did not start", () =>
        restarted.output.includes('"iteration":1'),
      );
      await producer.send({ topic, messages: [{ value }] });
      await waitFor(
        "restarted runtime did not commit duplicate offset",
        async () => (await offset()) === "2",
      );
      assert.deepEqual(
        await snapshot(),
        completed,
        "duplicate delivery must not claim or execute the task again",
      );
...
test("retention compacts worker history while preserving duplicate and conflict protection", async () => {
  const { runRetention } = await import("@pstack/database/retention");
  ...
  try {
    ...
    const taskBefore = (
      await pool.query("SELECT id FROM app_tasks WHERE object_type='async_task' AND object_id=$1", [
        task.taskId,
      ])
    ).rows[0];
    ...
    await pool.query("UPDATE app_task_events SET created_at='2000-01-01' WHERE task_id=$1", [
      taskBefore.id,
    ]);
    const retention = {
    ...
    assert.deepEqual(
      (await pool.query("SELECT result FROM app_async_receipts WHERE idempotency_key=$1", [key]))
        .rows[0].result,
      { effect: "once" },
    );
    assert.deepEqual(await runRetention({ ...retention, dryRun: false }), preview);
    ...
    const tombstone = (
      await pool.query("SELECT response_data,status FROM app_idempotency_keys WHERE key=$1", [key])
    ).rows[0];
    assert.equal(tombstone.response_data, null);
    assert.equal(tombstone.status, "succeeded");
    assert.equal(
      (
        await pool.query("SELECT count(*)::int AS n FROM app_task_events WHERE task_id=$1", [
          taskBefore.id,
        ])
      ).rows[0].n,
      0,
    ...
    assert.equal(
      (await pool.query("SELECT status FROM app_tasks WHERE id=$1", [taskBefore.id])).rows[0]
        .status,
      "succeeded",
    );
    const legacyAfter = (
      await pool.query("SELECT request_hash,response_data FROM app_idempotency_keys WHERE key=$1", [
        legacy.key,
      ])
    ).rows[0];
    ...
    assert.equal(
      (
        await processAsyncConsumerMessage(
          message(JSON.parse(input.value).eventId, { "\u00e9": 1, "e\u0301": 2 }),
          {
            ...options,
            store: recreated,
            handler,

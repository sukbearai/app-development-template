...
function launch(args, env) {
  ...
  const exited = new Promise((resolve, reject) => {
    ...
    child.once("close", (code, signal) => { result = { code, signal }; resolve(result); });
  });
  return { child, exited, get result() { return result; }, get stdout() { return stdout; }, get stderr() { return stderr; } };
}
async function runtime(t, overrides = {}, fixture, command = "outbox-loop") {
  ...
  const env = {
    ...process.env, NODE_ENV: "test", APP_TEMPLATE_WORKER_SKIP_ENV_FILES: "1",
    DATABASE_URL: database.toString(), OUTBOX_PUBLISHER: "dry-run", OUTBOX_DRY_RUN: "1",
    WORKER_HEARTBEAT_PATH: heartbeat, WORKER_SHUTDOWN_TIMEOUT_MS: "3000", ...overrides,
  };
...
async function lockTable(pool, t, table) {
  ...
  t.after(async () => { await client.query("ROLLBACK"); client.release(); });
  return client;
}

export function registerWorkerLifecycleTests(pool) {
  test("lifecycle reports a real consumer SQL failure after SIGTERM", { timeout: 20000 }, async (t) => {
    const topic = `drain-failure-${randomUUID()}`;
    const worker = await runtime(t, {
      OUTBOX_PUBLISHER: "kafka", OUTBOX_DRY_RUN: "0",
      KAFKA_BROKERS: process.env.WORKER_TEST_KAFKA_BROKERS,
      ASYNC_RUNTIME_TOPICS: topic, KAFKA_CONSUMER_GROUP_ID: topic,
      WORKER_SHUTDOWN_TIMEOUT_MS: "5000",
    }, undefined, "async-runtime");
    ...
    const producer = new Kafka({
      brokers: process.env.WORKER_TEST_KAFKA_BROKERS.split(","), logLevel: logLevel.NOTHING,
    }).producer();
    ...
    await waitFor("accepted quarantine INSERT blocked", async () => {
      backend = (await pool.query(
        `SELECT pid FROM pg_stat_activity WHERE application_name=$1 AND wait_event_type='Lock'
         AND query LIKE 'INSERT INTO "app_message_quarantine"%'`, [worker.applicationName],
      )).rows[0];
      return backend !== undefined;
    });
    worker.child.kill("SIGTERM");
    await waitFor("drain starts", async () => (await readHeartbeat(worker.heartbeat))?.state === "stopping");
    await pause(100);
    assert.equal(worker.result, undefined);
    assert.equal((await pool.query("SELECT pg_cancel_backend($1) cancelled", [backend.pid])).rows[0].cancelled, true);
    ...
    t.diagnostic(JSON.stringify({ injected: "pg_cancel_backend of accepted quarantine INSERT after SIGTERM", exit: worker.result, heartbeat: heartbeat.state }));
    assert.deepEqual(await worker.exited, { code: 1, signal: null }, worker.stderr);
    assert.equal(heartbeat.state, "failed");
    assert.equal((await pool.query("SELECT count(*)::int n FROM pg_stat_activity WHERE application_name=$1", [worker.applicationName])).rows[0].n, 0);
  });

  test("lifecycle startup remains unhealthy, then repeated SIGTERM drains accepted SQL", { timeout: 20000 }, async (t) => {
    ...
    worker.child.kill("SIGTERM");
    await waitFor("stopping heartbeat", async () => (await readHeartbeat(worker.heartbeat))?.state === "stopping");
    worker.child.kill("SIGTERM");
    await pause(100);
  ...
  test("lifecycle stops during startup without beginning an iteration", { timeout: 10000 }, async (t) => {
    const lock = await lockTable(pool, t, "app_kafka_recovery");
    const worker = await runtime(t);
    await waitFor("startup lock", () => blocked(pool, worker));
    worker.child.kill("SIGTERM");
    await waitFor("startup stopping", async () => (await readHeartbeat(worker.heartbeat))?.state === "stopping");
    worker.child.kill("SIGTERM");
    await lock.query("ROLLBACK");
  ...
  test("lifecycle waits for Kafka group join even when run resolves", { timeout: 15000 }, async (t) => {
    const worker = await runtime(t, {
      ASYNC_RUNTIME_TOPICS: `readiness-${randomUUID()}`,
      KAFKA_CONSUMER_GROUP_ID: `readiness-${randomUUID()}`,
    }, "consumer-no-join");
    await waitFor("consumer run returned", () => worker.stdout.includes("run returned without group join"));
    await pause(3200);
    assert.equal((await readHeartbeat(worker.heartbeat)).state, "starting");
    assert.equal(worker.stdout.includes('"iteration":'), false);
    worker.child.kill("SIGTERM");
  ...
  test("lifecycle finite completion cleans up and exits without a signal", { timeout: 10000 }, async (t) => {
  ...
  for (const table of ["app_kafka_recovery", "app_outbox_events"]) {
    test(`lifecycle deadline bounds ${table} lock and repeated signals`, { timeout: 15000 }, async (t) => {
      ...
      assert.deepEqual(await worker.exited, { code: 1, signal: null }, worker.stderr);
      assert.ok(elapsed >= 600 && elapsed < 1000, `deadline was bypassed or reset: ${elapsed} ms`);
      ...
      t.diagnostic(JSON.stringify({ table, elapsedMs: elapsed, exit: worker.result, heartbeat: "stopping" }));
    });
  }

  for (const mode of ["cleanup-hang", "falsy-hang", "cleanup-failure", "falsy-failure", "cleanup-reject-live", "connect-reject-hang"]) {

...
export function createHeartbeatWriter(
  destination = process.env.WORKER_HEARTBEAT_PATH ??
    join(tmpdir(), `pstack-worker-heartbeat-${process.pid}-${randomUUID()}.json`),
) {
      ...
      .then(async () => {
        try {
          ...
          try {
            ...
            throw new AggregateError([error, cleanupError], "Heartbeat write and cleanup failed");
          }
          throw error;
        }
      });
...
export async function inspectWorkerHeartbeat(destination = process.env.WORKER_HEARTBEAT_PATH) {
  if (!destination)
    throw new Error("WORKER_HEARTBEAT_PATH is required to inspect a worker instance");
  ...
  const status =
    record.state === "running" && ageMs < 15000 && progressAgeMs < 120000 ? "ok" : "degraded";
  return {
    status,
    service: "worker",
    pid: record.pid,

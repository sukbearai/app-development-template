...
新生成的 `release.json` 使用 `schemaVersion: 2`，要求包含 `conventions:check`。读取已发布清单时，校验器按清单版本使用固定门禁。版本不高于 `0.2.3` 的 v1 清单沿用当时的门禁，但仍核验全部文件哈希、源码身份、容器验证与安全证据。新增门禁时必须新增清单策略版本，不能修改旧版策略。候选预检和清单生成始终要求当前全部门禁。
...
仓库更名后，已发布制品的镜像和签名仍使用发布时的名称。前任制品和部署校验先确认两个角色的镜像使用同一历史仓库，再通过 GitHub API 确认旧名与当前名指向相同仓库 ID 和当前规范名称，最后按历史工作流身份核验全部签名与证明。查询失败或身份不一致会阻止校验。新制品签名仍只接受当前仓库名称。

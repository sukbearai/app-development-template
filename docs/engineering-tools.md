...
`Verify template` 保留必需检查名 `verify`。`scripts/ci-plan.mjs` 在安装依赖前读取事件提交和 Git 对象，将范围与来源写入 `artifacts/ci/plan.json`。未知路径、缺失或分叉的基线、读取错误均执行原有完整流程。

纯版本变更只允许根 `package.json`、`.release-please-manifest.json` 和 `CHANGELOG.md`。脚本比较提交前后的 JSON，确认根包除版本外完全相同，manifest 仍只有根版本键，然后执行格式、版本一致性和文档检查。依赖、包脚本、锁文件或其他配置变化进入完整流程。

工程增量仅允许脚本中明确列出的公开发布部署演练、对应测试、两个工作流、CI 选择器及两份文档。该范围执行格式、lint、重复代码、依赖、分层、供应链、契约、类型、迁移、版本、文档及全部工具测试。它不产生新的应用浏览器、构建或恢复测试结果；实际公开镜像的安装、升级与回滚仍由部署演练验证。工程与版本或运行时代码混合变更执行完整流程。

PR 检出事件中的准确 head 提交，基线使用事件中的 base SHA；分支保护应要求合并前更新到主分支。主分支 push 仅在 Git 树与已合并 PR head 完全相同，且只读 GitHub API 确认同仓库、同 PR head 的 `pull_request` 事件已通过 `Verify template` 和 `.github/workflows/ci.yml` 时复用该运行。证据记录原运行链接、编号、尝试次数和提交树，不把旧结果记为本次测试。任何证明缺失都恢复按变更范围执行。CI 范围判断不会减少发行门禁或替代实际镜像、签名和部署证据。

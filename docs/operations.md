...
Run `pnpm template:init --name my-project` to rename the root package, synchronize `APP_NAME` in `.env.example` and `.env`, and create `.env` when it is missing. Existing credentials and other `.env` settings stay unchanged. Workspace package names stay stable. The initializer does not copy maintenance history, credentials or previous project state.
...
The Web launcher installs SIGTERM and SIGINT handling before asynchronous application startup. A stop request refuses new tracked work immediately, waits for initialization already in progress, closes any late server, and drains admitted HTTP requests and API operations. An API operation remains active until its promise settles, including when its client disconnects. The launcher then closes registered clients once. A thrown startup error uses the same cleanup path. `WEB_SHUTDOWN_TIMEOUT_MS` defaults to 30000 and bounds the entire stop sequence from the first signal or failure, including remaining startup and cleanup. Timeout forces exit 1. Repeated signals do not extend the deadline or bypass cleanup.
...
Run `pnpm production:check` against the intended deployment environment before promotion. This explicit policy checks configuration without changing local production-mode verification. See [production deployment](production-deployment.md) for the supplied environment templates and [target acceptance](production-acceptance.md) for the remaining live checks.
...
`UPLOAD_MAX_CONCURRENT` limits admitted uploads per Web process and defaults to 2, with a supported range of 1 through 64. Admission begins after authorization and before multipart buffering. The slot remains occupied until storage, database finalization and failure reconciliation settle, including after client disconnect. Excess uploads receive HTTP 503 with `UPLOAD_BUSY` and `Retry-After: 1`; they are not queued. Size limits remain separate. Measure concurrent-upload memory with `pnpm test:capacity` before increasing this limit.

`GET /api/system/metrics` requires a separate `METRICS_TOKEN` bearer credential. Omitted or empty configuration disables access; user sessions and service tokens do not grant it. The JSON contract exposes per-process HTTP counters, memory, pool and upload admission state, plus cached shared-database aggregates. Failed database observations are marked unavailable. `pnpm monitor:check` evaluates one observation and exits nonzero for alert conditions or collection errors. See [monitoring](monitoring.md) for thresholds, instance aggregation and recipient acceptance.
...
Worker startup reports `starting` until required initialization completes. For consumers, the first Kafka group join and completed consumer initialization precede `running`. Stop requests report `stopping`; successful cleanup reports `stopped`, while failures report `failed` when heartbeat persistence succeeds. Only fresh running heartbeats can pass `health --live`. The first stop or fatal failure starts the absolute `WORKER_SHUTDOWN_TIMEOUT_MS` deadline, default 30000 ms. Repeated SIGTERM/SIGINT do not reset it. Compose's `WORKER_STOP_GRACE_PERIOD` defaults to 40s and must exceed that deadline. Forced expiry exits 1 without pretending that terminal heartbeat or cleanup completed.

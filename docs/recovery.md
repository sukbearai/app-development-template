...
Every publishing command, including `outbox-once` and `outbox-loop`, validates the recovery binding and current Kafka transport offsets before claiming each event. A failed check leaves that event unclaimed. Restored deployments open and close a Kafka admin connection for each check; deployments without a recovery binding do not. Explicit dry-run remains a database-only preview.

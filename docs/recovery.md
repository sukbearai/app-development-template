...
Every publishing command, including `outbox-once` and `outbox-loop`, validates the recovery binding and current Kafka transport offsets before claiming each event. A failed check leaves that event unclaimed. Long-running workers own the Kafka admin connection through shutdown and reuse it for recovery checks. Standalone publishing checks close the admin connection they allocate. Deployments without a recovery binding do not connect an admin client for this validation. Explicit dry-run remains a database-only preview.

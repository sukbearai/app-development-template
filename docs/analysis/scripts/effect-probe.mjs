...
const Api = HttpApi.make("analysis-api").add(
  HttpApiGroup.make("hello").add(
    HttpApiEndpoint.post("echo", "/api/echo", { payload: Greeting, success: Greeting }),
  ),
);
const handlers = HttpApiBuilder.group(Api, "hello", (handlers) =>
  handlers.handle("echo", ({ payload }) => Effect.succeed(payload)),
);
...
checks.push(
  "One HttpApi schema produces a Web Request handler, validates payload, encodes response, and generates OpenAPI",
);
const output = {
  version: JSON.parse(readFileSync(new URL("node_modules/effect/package.json", import.meta.url)))
    .version,
  checks,
  statuses,
  boundary:
    "Published effect 4.0.0-rc.112 package, in-process Web Request/Response; no vinext integration, browser, database, Kafka, or network listener.",
};

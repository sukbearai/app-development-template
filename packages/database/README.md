...
Migration `0005_stale_marten_broadcloak` adds the file continuation index and the session invalidation index. It does not delete rows or change existing columns. PostgreSQL builds these indexes with ordinary `CREATE INDEX`; schedule the migration for a suitable maintenance window on a large existing database because writes can wait for index creation. Back up and verify the target before applying migrations. Older application code can use the database with these additional nonunique indexes; do not rewrite the migration ledger during application rollback.
...
Idempotency envelope compaction requires a valid `v2:` SHA-256 identity. Legacy envelopes remain available because their payload is needed to compare redeliveries independently of the old locale-sensitive hash. Legacy hashes and receipt identities are not rewritten. See the [worker upgrade boundaries](../../services/worker/README.md) before replacing consumers or choosing a rollback version.

...
const partition = z
  ...
  .strict();
export const kafkaCheckpointSchema = z
  .object({
    kind: z.literal("checkpoint"),
    clusterId: z.string().min(1),
    logicalGroup: group,
    sourceTransportGroup: group,
    partitions: z.array(partition).min(1),
    topics: z
      .array(z.object({ topic: z.string().min(1), cleanupPolicy: z.literal("delete") }).strict())
      .min(1),
  })
  .strict()
  .superRefine((value, context) => {
    ...
    if (
      ...
      value.partitions.some(
        (row) =>
          !names.includes(row.topic) ||
          BigInt(row.low) > BigInt(row.nextOffset) ||
          BigInt(row.nextOffset) > BigInt(row.high),
      ) ||
...
export const kafkaRecoverySchema = z.discriminatedUnion("kind", [
...
export type PartitionOffset = Pick<
  KafkaCheckpoint["partitions"][number],
  "topic" | "partition" | "nextOffset"
>;
const key = (row: { topic: string; partition: number }) =>
  JSON.stringify([row.topic, row.partition]);
...
async function inspectLogs(admin: Admin, topics: string[]) {
  ...
  if (topics.some((topic) => !existing.includes(topic)))
    throw new Error("Kafka recovery topic is missing");
  ...
  for (const topic of topics) {
    ...
    if (
      !resource ||
      resource.errorCode ||
      resource.configEntries.find((entry) => entry.configName === "cleanup.policy")?.configValue !==
        "delete"
    ) {
  ...
  const partitions = (
    await Promise.all(
      topics.map(async (topic) =>
        (await admin.fetchTopicOffsets(topic)).map((row) => ({
          topic,
          partition: row.partition,
          low: offset.parse(row.low),
          high: offset.parse(row.high),
        })),
      ),
    )
  ).flat();
...
export async function readTransportOffsets(
  admin: Admin,
  topics: string[],
  transportGroup: string,
): Promise<PartitionOffset[]> {
  const offsets = await admin.fetchOffsets({
    groupId: transportGroup,
    topics,
    resolveOffsets: false,
  });
  return offsets.flatMap(({ topic, partitions }) =>
    partitions.map((row) => ({ topic, partition: row.partition, nextOffset: row.offset })),
  );
}

export async function captureKafkaCheckpoint(
  admin: Admin,
  logicalGroup: string,
  transportGroup: string,
  topics: string[],
): Promise<KafkaCheckpoint> {
  group.parse(logicalGroup);
  group.parse(transportGroup);
  ...
  return kafkaCheckpointSchema.parse({
    kind: "checkpoint",
    clusterId: logs.clusterId,
    logicalGroup,
    sourceTransportGroup: transportGroup,
...
export async function validateKafkaHistory(
  admin: Admin,
  checkpoint: KafkaCheckpoint,
  required: PartitionOffset[] = checkpoint.partitions,
) {
  const logs = await inspectLogs(
    admin,
    checkpoint.topics.map((row) => row.topic),
  );
  if (logs.clusterId !== checkpoint.clusterId)
    throw new Error("Kafka recovery cluster differs from the checkpoint");
  if (
    logs.partitions.length !== checkpoint.partitions.length ||
    logs.partitions.some((row) => !checkpoint.partitions.some((saved) => key(saved) === key(row)))
  )
    throw new Error("Kafka recovery partition topology changed");
  for (const row of required) {
    ...
    if (
      ...
      !saved ||
      next < BigInt(log.low) ||
      next > BigInt(log.high) ||
      BigInt(log.high) < BigInt(saved.high)
    ) {
      throw new Error(
        `Kafka recovery history unavailable: ${key(row)} next=${row.nextOffset} low=${log?.low} high=${log?.high}`,
      );
    }
  }
}

export async function verifyRecoveryTransport(
  admin: Admin,
  checkpoint: KafkaCheckpoint,
  transportGroup: string,
) {
  const current = await readTransportOffsets(
    admin,
    checkpoint.topics.map((row) => row.topic),
    transportGroup,
  );
  if (current.length !== checkpoint.partitions.length)
    throw new Error("Kafka recovery transport offsets are incomplete");
  for (const saved of checkpoint.partitions) {
    const found = current.find((row) => key(row) === key(saved));
    if (
      !found ||
      !offset.safeParse(found.nextOffset).success ||
      BigInt(found.nextOffset) < BigInt(saved.nextOffset)
    ) {
  ...
  if (
    groups.length !== 1 ||
    !["Empty", "Stable", "PreparingRebalance", "CompletingRebalance"].includes(
      groups[0]?.state || "",
    ) ||
...
export async function initializeRecoveryTransport(
  admin: Admin,
  checkpoint: KafkaCheckpoint,
  transportGroup: string,
) {
  if (
    transportGroup === checkpoint.sourceTransportGroup ||
    transportGroup === checkpoint.logicalGroup
  )
    throw new Error("Kafka recovery requires a fresh transport group");
  if ((await admin.listGroups()).groups.some((row) => row.groupId === transportGroup))
    throw new Error("Kafka recovery transport group already exists");
  const existing = await readTransportOffsets(
    admin,
    checkpoint.topics.map((row) => row.topic),
    transportGroup,
  );
  if (existing.some((row) => row.nextOffset !== "-1"))
    throw new Error("Kafka recovery transport has existing offsets");
  ...
  for (const { topic } of checkpoint.topics) {
    await admin.setOffsets({
      ...
      partitions: checkpoint.partitions
        ...
        .map((row) => ({ partition: row.partition, offset: row.nextOffset })),
    });
  }
  ...
  if (
    actual.some(
      (row) =>
        row.nextOffset !==
        checkpoint.partitions.find((saved) => key(saved) === key(row))?.nextOffset,
    )
  )
    throw new Error("Kafka recovery transport initialization readback differs");
}

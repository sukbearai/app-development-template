...
function broker({ low = "0", high = "10", committed = "10", advance = false } = {}) {
  return {
    async describeCluster() {
      return { clusterId: "checkpoint-test" };
    },
    async listTopics() {
      return ["app.tasks"];
    },
...
test("checkpoint remains replayable when publishing and consumption advance during capture", async () => {
  const checkpoint = await captureKafkaCheckpoint(
    broker({ advance: true }),
    "logical",
    "transport",
    ["app.tasks"],
  );
  assert.deepEqual(checkpoint.partitions, [
    { topic: "app.tasks", partition: 0, low: "0", high: "10", nextOffset: "10" },
  ]);
});
...
test("checkpoint still refuses committed offsets outside retained history", async () => {
  for (const options of [
    { low: "5", committed: "4" },
    { high: "10", committed: "11" },
    { low: "5", committed: "-1" },
  ]) {
    await assert.rejects(
      captureKafkaCheckpoint(broker(options), "logical", "transport", ["app.tasks"]),
    );
  }
  const fresh = await captureKafkaCheckpoint(broker({ committed: "-1" }), "logical", "transport", [
    "app.tasks",
  ]);

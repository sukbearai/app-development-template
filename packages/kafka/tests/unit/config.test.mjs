...
test("plaintext defaults preserve broker parsing and reject misplaced security settings", () => {
  assert.deepEqual(readKafkaConfig(base), {
    brokers: ["first:9092", "second:9092"],
    clientId: "app-template-worker",
  });
  for (const protocol of ["SASL_PLAINTEXT", "ssl", "", "secret-invalid-value"])
    assert.throws(
      () => readKafkaConfig({ ...base, KAFKA_SECURITY_PROTOCOL: protocol }),
      /KAFKA_SECURITY_PROTOCOL must/,
    );
...
test("TLS always verifies server identity and supports system roots and paired client certificates", async () => {
  ...
  for (const key of ["KAFKA_SSL_CERT_FILE", "KAFKA_SSL_KEY_FILE"])
    assert.throws(
      () => readKafkaConfig({ ...env, [key]: "/private/secret" }),
      /configured together/,
    );
  assert.throws(
    () => readKafkaConfig({ ...env, KAFKA_SASL_PASSWORD: "secret" }),
    /require SASL_SSL/,
  );
  ...
  try {
    ...
    assert.deepEqual(readKafkaConfig(env).ssl, {
    ...
    for (const kind of ["CA", "CERT", "KEY"]) {
      ...
      assert.throws(
        () => readKafkaConfig({ ...env, [field]: "/private/secret/missing" }),
        (error) => error.message === `${field} could not be read` && error.cause === undefined,
      );
    }
  } finally {
...
test("SASL requires TLS, an explicit supported mechanism and both credentials", () => {
  ...
  for (const mechanism of [undefined, "PLAIN", "oauthbearer", "scram-sha256"])
    assert.throws(
      () => readKafkaConfig({ ...env, KAFKA_SASL_MECHANISM: mechanism }),
      /KAFKA_SASL_MECHANISM must/,
    );
  for (const key of ["KAFKA_SASL_USERNAME", "KAFKA_SASL_PASSWORD"])
    assert.throws(
      () => readKafkaConfig({ ...env, KAFKA_SASL_MECHANISM: "plain", [key]: "" }),

...
import { assertNever, asyncTaskEventMessageSchema, kafkaConsumerOffsetSchema, loginRequestSchema, telemetryRequestSchema } from "../src/index.ts";
...
test("async message contracts bound indexed identifiers by UTF-8 bytes", () => {
  ...
  for (const field of ["eventId", "eventType", "traceId", "taskId", "idempotencyKey"]) {
    assert.equal(asyncTaskEventMessageSchema.safeParse({ ...input, [field]: "😀".repeat(500) }).success, true);
    assert.equal(asyncTaskEventMessageSchema.safeParse({ ...input, [field]: "😀".repeat(501) }).success, false);
    assert.equal(asyncTaskEventMessageSchema.safeParse({ ...input, [field]: "x".repeat(2001) }).success, false);
  }
});
...
test("Kafka metadata contracts keep quarantine composite indexes bounded without rewriting group identity", () => {
  ...
  assert.equal(kafkaConsumerOffsetSchema.safeParse({ ...offset, consumerGroup: "😀".repeat(64) }).success, true);
  for (const extra of [{ consumerGroup: "😀".repeat(65) }, { consumerGroup: "x\u0000" }, { consumerGroup: "x\ud800" }, { topic: "x".repeat(250) }, { offset: "x" }, { offset: "1".repeat(20) }])

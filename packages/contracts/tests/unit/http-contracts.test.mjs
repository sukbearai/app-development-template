import {
  ...
  userSchema,
  loginResponseSchema,
} from "../../src/modules/identity/contracts.ts";
...
import { uploadRequestSchema } from "../../src/modules/uploads/contracts.ts";
import { currentUserResponseSchema } from "../../src/modules/identity/contracts.ts";

import { telemetryRequestSchema } from "../../src/modules/telemetry/contracts.ts";
import { asyncTaskEventMessageSchema } from "../../src/async-contracts.ts";
import { outboxEventSchema } from "../../src/modules/outbox/contracts.ts";
...
test("request defaults are optional in OpenAPI input and required after parsing", () => {
  for (const [name, schema, input, defaults] of [
    [
      "CreateUserRequest",
      createUserRequestSchema,
      { account: "alice", displayName: "Alice", password: "12345678" },
      ["roleIds", "status"],
    ],
    [
      "CreateRoleRequest",
...
test("health 503 is the health data envelope, never an API failure envelope", () => {
  ...
  assert.equal(
    doc.paths["/api/system/health"].get.responses[503].content["application/json"].schema.$ref,
  ...
  for (const operation of apiOperations) {
    for (const [status, response] of Object.entries(operation.responses)) {
      ...
      if (Number(status) < 400) {
        assert.throws(() =>
          parseApiResponse(operation.operationId, Number(status), wrap({ arbitrary: true })),
        );
      }
    }
  }
...
test("external REST auth, permission, rate and size statuses are declared", () => {
  for (const [operationId, statuses] of [
    ...
    for (const status of statuses)
      assert.deepEqual(parseApiResponse(operationId, status, failure), failure);
  }
  assert.throws(
...
test("message boundaries preserve required ownership and retry fields", () => {
  ...
  assert.equal(
    asyncTaskEventMessageSchema.safeParse({ ...message, attemptCount: 0 }).success,
    false,
  );
  const outbox = {
    id: "outbox_1",
...
test("generic success constructor validates its concrete payload", () => {
  ...
  assert.equal(
    apiOperation("getApiHello").responses[200].schema.safeParse({ message: "Hello from vinext" })
      .success,
    true,
  );
});
...
test("existing credentials above 256 characters remain usable for login and rotation", () => {
  ...
  assert.equal(
    createUserRequestSchema.safeParse({ account: "new", displayName: "New", password }).success,
    false,
  );
});

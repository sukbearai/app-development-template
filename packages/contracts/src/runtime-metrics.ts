...
export const databaseMetricsSchema = z.discriminatedUnion("status", [
  z.object({
    status: z.literal("available"), observedAt: z.iso.datetime(),
    outbox: z.object({ pending: count, processing: count, failed: count, deadLetter: count,
      published: count, oldestPendingAgeMs: age, staleLocks: count }),
    tasks: z.object({ pending: count, running: count, succeeded: count, failed: count,
      deadLetter: count, canceled: count, oldestUnfinishedAgeMs: age }),
...
export const runtimeMetricsSchema = z.object({
  ...
  http: z.array(z.object({ operationId: z.string().max(80), status: z.number().int().min(100).max(599),
    count, durationMsTotal: age, durationMsMax: age })).max(512),

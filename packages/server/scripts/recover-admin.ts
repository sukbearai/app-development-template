...
try {
  ...
  const { values } = parseArgs({ options: { account: { type: "string" }, confirm: { type: "boolean" } }, strict: true, allowPositionals: false });
  ...
  const result = await recoverAdministrator({ account: values.account, newPassword, confirm: true });
  process.stdout.write(`${JSON.stringify(result)}\n`);
} catch {
  process.stderr.write("Administrator recovery failed. Check the account, privileges, password policy and explicit confirmation.\n");
  process.exitCode = 1;
} finally {
  await closeDatabase();
}

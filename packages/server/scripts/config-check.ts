...
if (args.length > 1 || (args.length === 1 && args[0] !== "--deployment")) {
  ...
  try {
    ...
    if (args[0] === "--deployment") {
      ...
      if (!parsed.success)
        issues.push(...parsed.error.issues.map((issue) => `${String(issue.path[0] ?? "environment")}: invalid configuration value`));
      if (parsed.success && parsed.data.WEB_REPLICAS > 1 && parsed.data.RATE_LIMIT_DRIVER !== "redis")
      ...
      if (issues.length) {
        ...
        process.stdout.write(`Configuration valid (${config.NODE_ENV}, ${config.UPLOAD_STORAGE_DRIVER}, ${config.RATE_LIMIT_DRIVER})\n`);
      }
    }
  } catch {

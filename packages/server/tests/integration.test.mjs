    ...
    try {
      ...
      await t.test(
        ...
        async () => {
          ...
          const { createPostgresAsyncTaskStore, processAsyncConsumerMessage } =
      ...
      await t.test(
        ...
        async () => {
          ...
          assert.equal(await product.reconcileUploadIntent(good.storageKey), "protected");
          await query(
        ...
        async () => {
          ...
          for (const fileName of ["", "   ", "\t"]) {
            const response = await withAccessLog(
              ...
              async () =>
                ok(
                  await product.storeUploadedFile({
                    token: adminToken,
...
test(
  ...
  async () => {
    ...
    const { redisWindowCount, redisCommand, closeRedis } = await import("../src/redis-client.ts");
    try {
      for (let i = 0; i < 30; i++) {
        try {

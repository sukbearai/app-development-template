...
const s3 = {
  ...
  OBJECT_STORAGE_ENDPOINT: "https://objects.acme.internal",
  OBJECT_STORAGE_ACCESS_KEY: "provisioned-access-key",
  OBJECT_STORAGE_SECRET_KEY: "provisioned-secret-key",
...
for (const [name, values, key, code] of [
  [
    "metrics placeholder",
    { METRICS_TOKEN: "replace_me_with_a_random_metrics_token" },
    "METRICS_TOKEN",
  ...
  [
    "explicit insecure cookie",
    { SESSION_COOKIE_SECURE: "false" },
    "SESSION_COOKIE_SECURE",
    "SECURE_COOKIE_REQUIRED",
  ],
  [
    "invalid cookie override",
    { SESSION_COOKIE_SECURE: "perhaps" },
    "SESSION_COOKIE_SECURE",
    "INVALID_VALUE",
  ],
  ...
  [
    "database fragment",
    { DATABASE_URL: `${local.DATABASE_URL}#secret` },
    "DATABASE_URL",
    "INVALID_DATABASE_URL",
  ],
  [
    "database query override",
    { DATABASE_URL: `${local.DATABASE_URL}?password=changeme` },
    "DATABASE_URL",
    "INVALID_DATABASE_URL",
  ],
  [
    "invalid database URL",
    { DATABASE_URL: "https://db/app" },
    "DATABASE_URL",
    "INVALID_DATABASE_URL",
  ],
  [
    "database placeholder",
    { DATABASE_URL: "postgres://app:local-development-only@db/app" },
    "DATABASE_URL",
    "PLACEHOLDER_CREDENTIAL",
  ],
  [
    "encoded placeholder",
    { DATABASE_URL: "postgres://app:%63hangeme@db/app" },
    "DATABASE_URL",
    "PLACEHOLDER_CREDENTIAL",
  ],
  [
    "invalid URL escape",
    { DATABASE_URL: "postgres://app:%zz@db/app" },
    "DATABASE_URL",
    "INVALID_URL",
  ],
  ...
  [
    "multiple replicas need Redis",
    { WEB_REPLICAS: "2" },
    "RATE_LIMIT_DRIVER",
    "SHARED_RATE_LIMIT_REQUIRED",
  ],
  [
    "multiple replicas need shared storage",
    { WEB_REPLICAS: "2", RATE_LIMIT_DRIVER: "redis", REDIS_URL: "redis://cache:6379" },
    "UPLOAD_STORAGE_SHARED",
    "SHARED_STORAGE_REQUIRED",
  ],
  ["Redis missing URL", { RATE_LIMIT_DRIVER: "redis" }, "REDIS_URL", "REQUIRED"],
  [
    "Redis invalid database",
    { RATE_LIMIT_DRIVER: "redis", REDIS_URL: "redis://cache/invalid" },
    "REDIS_URL",
    "INVALID_URL",
  ],
  [
    "Redis wrong protocol",
    { RATE_LIMIT_DRIVER: "redis", REDIS_URL: "https://cache" },
    "REDIS_URL",
    "INVALID_URL",
  ],
  [
    "Redis placeholder",
    { RATE_LIMIT_DRIVER: "redis", REDIS_URL: "redis://:changeme@cache:6379" },
    "REDIS_URL",
    "PLACEHOLDER_CREDENTIAL",
  ],
  [
    "invalid concurrency",
    { UPLOAD_MAX_CONCURRENT: "65" },
  ...
  [
    "invalid shared boolean",
    { UPLOAD_STORAGE_SHARED: "yes" },
    "UPLOAD_STORAGE_SHARED",
    "INVALID_VALUE",
  ],
  ["invalid drain", { WEB_SHUTDOWN_TIMEOUT_MS: "0" }, "WEB_SHUTDOWN_TIMEOUT_MS", "INVALID_VALUE"],
  [
    "invalid worker drain",
    { WORKER_SHUTDOWN_TIMEOUT_MS: "0" },
    "WORKER_SHUTDOWN_TIMEOUT_MS",
    "INVALID_VALUE",
  ],
  [
    "worker drain exceeds limit",
    { WORKER_SHUTDOWN_TIMEOUT_MS: "300001" },
    "WORKER_SHUTDOWN_TIMEOUT_MS",
    "INVALID_VALUE",
  ],
  [
    "worker grace below drain",
    { WORKER_STOP_GRACE_PERIOD: "10s" },
    "WORKER_STOP_GRACE_PERIOD",
    "INSUFFICIENT_GRACE",
  ],
  [
    "worker grace equals drain",
    { WORKER_SHUTDOWN_TIMEOUT_MS: "40000", WORKER_STOP_GRACE_PERIOD: "40s" },
    "WORKER_STOP_GRACE_PERIOD",
    "INSUFFICIENT_GRACE",
  ],
  [
    "omitted worker grace below custom drain",
    { WORKER_SHUTDOWN_TIMEOUT_MS: "60000" },
    "WORKER_STOP_GRACE_PERIOD",
    "INSUFFICIENT_GRACE",
  ],
  [
    "empty worker grace uses Compose default",
    { WORKER_SHUTDOWN_TIMEOUT_MS: "60000", WORKER_STOP_GRACE_PERIOD: "" },
    "WORKER_STOP_GRACE_PERIOD",
    "INSUFFICIENT_GRACE",
  ],
  [
    "omitted web grace below custom drain",
    { WEB_SHUTDOWN_TIMEOUT_MS: "60000" },
    "WEB_STOP_GRACE_PERIOD",
    "INSUFFICIENT_GRACE",
  ],
  [
    "grace below default drain",
    { WEB_STOP_GRACE_PERIOD: "10s" },
    "WEB_STOP_GRACE_PERIOD",
    "INSUFFICIENT_GRACE",
  ],
  [
    "grace equals drain",
    { WEB_SHUTDOWN_TIMEOUT_MS: "40000", WEB_STOP_GRACE_PERIOD: "40s" },
    "WEB_STOP_GRACE_PERIOD",
    "INSUFFICIENT_GRACE",
  ],
  ["invalid publisher", { OUTBOX_PUBLISHER: "typo" }, "OUTBOX_PUBLISHER", "INVALID_VALUE"],
  [
    "diagnostic Kafka selection",
    { ASYNC_RUNTIME_PUBLISHER: "kafka" },
    "KAFKA_SECURITY_PROTOCOL",
    "TLS_REQUIRED",
  ],
  [
    "Kafka missing brokers",
    { OUTBOX_PUBLISHER: "kafka", KAFKA_SECURITY_PROTOCOL: "SSL" },
    "KAFKA_BROKERS",
    "INVALID_KAFKA_CONFIG",
  ],
  [
    "plaintext Kafka",
    { OUTBOX_PUBLISHER: "kafka", KAFKA_BROKERS: "broker:9092" },
    "KAFKA_SECURITY_PROTOCOL",
    "TLS_REQUIRED",
  ],
  [
  ...
  [
    "Kafka unreadable certificate",
    {
      OUTBOX_PUBLISHER: "kafka",
      KAFKA_BROKERS: "broker:9092",
      KAFKA_SECURITY_PROTOCOL: "SSL",
      KAFKA_SSL_CA_FILE: "/nonexistent/private-certificate",
    },
    "KAFKA_BROKERS",
    "INVALID_KAFKA_CONFIG",
  ],
  ...
  test(`deployment rejects ${name}`, () => {
    ...
    assert.ok(
      issues.some((issue) => issue.key === key && issue.code === code),
...
test("deployment accepts explicit local, S3, or attested shared local configurations without NODE_ENV", () => {
  for (const config of [
    local,
    s3,
    { ...local, DATABASE_URL: "postgres://postgres:generated-password@db/app?sslmode=require" },
    { ...s3, UPLOAD_STORAGE_DRIVER: "local", UPLOAD_STORAGE_SHARED: "true" },
    ...
    },
    {
      ...
      KAFKA_SASL_PASSWORD: "generated-kafka-credential",
    },
  ])
...
test("selected S3 needs explicit config and deployed credentials", () => {
  for (const key of [
    ...
    assert.ok(
      assessDeploymentConfig({ ...s3, [key]: undefined }).some(
        (issue) => issue.key === key && issue.code === "REQUIRED",
      ),
    );
  assert.ok(
    assessDeploymentConfig({ ...s3, OBJECT_STORAGE_SECRET_KEY: "local-development-only" }).some(
      (issue) => issue.code === "PLACEHOLDER_CREDENTIAL",
    ),
  );
  assert.ok(
    assessDeploymentConfig({ ...s3, OBJECT_STORAGE_ENDPOINT: "ftp://objects" }).some(
      (issue) => issue.code === "INVALID_STORAGE_URL",
    ),
  );
});
  ...
  for (const args of [[], ["--deployment"]]) {
    for (const env of [
      ...
      { ...local, APP_ORIGIN: `https://user:${secret}@private.invalid/path`, DATABASE_URL: secret },
      {
        ...
        KAFKA_BROKERS: "broker:9092",
        KAFKA_SECURITY_PROTOCOL: "SSL",
        KAFKA_SSL_CA_FILE: `/private/${secret}`,
      },
    ]) {
...
test("ordinary local production CLI policy remains compatible", () => {
  for (const env of [
    ...
    {
      NODE_ENV: "production",
      DATABASE_URL: "postgres://app:local-development-only@localhost/app",

...
import { buildAsyncRuntimeHealthSnapshot } from "../../src/modules/runtime/service.ts";
import { buildRuntimePlanFromEnv } from "../../src/runtime-health-config.ts";
...
test("production diagnostics keep bounded causes and source locations without error payloads", () => {
  const source = `
    ...
    const location = ${JSON.stringify(new URL("../../src/modules/identity/service.ts", import.meta.url).pathname)};
  ...
  assert.deepEqual(result.direct.frames[0], {
    file: "packages/server/src/modules/identity/service.ts",
    line: 42,
    column: 9,
  });

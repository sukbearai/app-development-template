...
test(
  "dedicated runtime metrics and upload admission against owned PostgreSQL",
  { timeout: 90000 },
  async (t) => {
    ...
    try {
      ...
      Object.assign(process.env, {
        ...
        UPLOAD_STORAGE_DIR: path.join(directory, "storage"),
      ...
      execFileSync("pnpm", ["--filter", "@pstack/database", "db:migrate"], {
        ...
        env: {
          ...process.env,
          TSX_TSCONFIG_PATH: path.resolve(import.meta.dirname, "../../../apps/web/tsconfig.json"),
        },
        stdio: "pipe",
      });
      ...
      const form = () => {
        ...
        body.set("file", new File(["hello"], "metrics.txt"));
        return body;
      };

      await t.test(
        "missing, wrong, oversized and administrator credentials never query the database",
        async () => {
          let acquired = 0;
      ...
      await t.test("fixed database facts, single-flight cache, and fresh pool queue", async () => {
        await query(
          "insert into app_tasks(id,task_type,status,trace_id,created_at) values('metric-task','secret-type','running','secret-trace',now()-interval '1 hour')",
        );
        await query(
          "insert into app_outbox_events(id,topic,event_type,payload,trace_id,created_at) values('metric-outbox','secret-topic','test','{}','secret-trace',now()-interval '1 hour')",
        );
        await query(
          "insert into app_upload_intents(id,storage_key,provider,state) values('metric-intent','secret-path','local','blocked')",
        );
        await query(
          "insert into app_message_quarantine(id,consumer_group,topic,partition,source_offset,error_code,error_message) values('metric-q','secret-group','secret-topic',0,'0','bad','secret-error')",
        );
        await query(
          "insert into app_async_recovery_quarantine(idempotency_key,consumer_group,original_record,error_code,error_message) values('secret-key','secret-group','{}','bad','secret-error')",
        );
        let acquired = 0;
        const observe = () => acquired++;
        database.getPool().on("acquire", observe);
        ...
        assert.equal(
          first.database.outbox.pending,
          Number(
            (await query("select count(*) from app_outbox_events where status='pending'")).rows[0]
              .count,
          ),
        );
        ...
        const clients = await Promise.all(
          Array.from({ length: 10 }, () => database.getPool().connect()),
        );
      ...
      await t.test(
        "stale outbox metrics match reclaimable leases including legacy null timestamps",
        async () => {
        ...
        from (values
          ...
          try {
            ...
            await query("delete from app_outbox_events where trace_id='lease-fixture'");
          }
        },
      );
      ...
      await t.test(
        "overflow rejects before body buffering and disconnect keeps occupied slot through commit",
        async () => {
          ...
          try {
            await until(
              async () =>
                Number(
                  (await query("select count(*) from app_upload_intents where state='writing'"))
                    .rows[0].count,
                ) === 1,
            );
            controller.abort();
          ...
          assert.equal(uploadAdmissionSnapshot().active, 0);
          assert.equal(
            Number((await query("select count(*) from app_file_assets")).rows[0].count),
            1,
          );
        },
      );
      await t.test("malformed, oversized, stream and storage failures release slots", async () => {
        ...
        large.set("file", new File(["a".repeat(1025)], "large.txt"));
        ...
        assert.equal(
          Number(
            (await query("select count(*) from app_upload_intents where state='cleanup'")).rows[0]
              .count,
          ),
          1,
        );
      ...
      await t.test(
        "commit failure settles durable reconciliation before slot release",
        async () => {
          await query(
            "create function reject_metric_file() returns trigger language plpgsql as $$ begin raise exception 'fixture rejection'; end $$; create trigger reject_metric_file before insert on app_file_assets for each row execute function reject_metric_file()",
          );
          try {
            assert.equal((await upload(form())).status, 500);
            assert.equal(uploadAdmissionSnapshot().active, 0);
            assert.equal(
              Number(
                (await query("select count(*) from app_upload_intents where state='deleted'"))
                  .rows[0].count,
              ),
              1,
            );
            ...
            await query(
              "drop trigger reject_metric_file on app_file_assets; drop function reject_metric_file()",
            );
          }
        },
      );
      await t.test(
        "database failures are cached unavailable observations and recover after expiry",
        async () => {
          await pause(1100);
          ...
          const uploadCounters = recovered.http.filter(
            (row) => row.operationId === "postApiUploads",
          );

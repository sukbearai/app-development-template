...
test("health route bounds real database probes and refreshes failed snapshots", { timeout: 60000 }, async (t) => {
  const name = `pstack-health-db-${randomUUID()}`;
  let control;
  let closeDatabase;
  try {
    docker("run", "-d", "--rm", "--name", name, "-e", "POSTGRES_PASSWORD=isolated-test-only", "-e", "POSTGRES_DB=health_test", "-p", "127.0.0.1::5432", "postgres:17-bullseye");
    const port = docker("port", name, "5432/tcp").split(":").at(-1);
    const address = `127.0.0.1:${port}/health_test`;
    control = new pg.Pool({ connectionString: `postgres://postgres:isolated-test-only@${address}` });
    for (let attempt = 0; ; attempt++) {
      try { await control.query("select 1"); break; }
      catch (error) { if (attempt === 60) throw error; await pause(100); }
    }
    ...
    Object.assign(process.env, {
      ...
      NODE_ENV: "test", APP_ENV: "test", APP_ORIGIN: "http://localhost",
      LOG_LEVEL: "error", RATE_LIMIT_DRIVER: "memory", WEB_REPLICAS: "1",
      UPLOAD_STORAGE_DRIVER: "local", OUTBOX_PUBLISHER: "disabled",
    });
    ...
    const request = async () => {
      ...
      const response = await GET(new Request("http://localhost/api/system/health", { headers: { "x-trace-id": traceId } }));
      ...
      return { status: response.status, health: body.data, cacheControl: response.headers.get("cache-control") };
    };
    ...
    await t.test("expired healthy and degraded snapshots wait for current database state", async () => {
      ...
      assert.deepEqual(await request(), failed, "degraded observations also have a short cache lifetime");
      await pause(1100);
      const before = probes;
    ...
    await t.test("unexpected snapshot rejection releases the in-flight slot", async (t) => {
      await pause(1100);
      const before = probes;
      t.mock.method(Date.prototype, "toISOString", () => { throw new Error("isolated health snapshot fault"); }, { times: 1 });
      const results = await Promise.allSettled(Array.from({ length: 10 }, healthCheck));
      assert.ok(results.every(result => result.status === "rejected" && result.reason.message === "isolated health snapshot fault"));
      assert.equal(probes - before, 1);
    ...
    await t.test("slow probes stay single-flight and TTL starts after completion", async () => {
      ...
      try {
        ...
        assert.equal(pool.waitingCount, 1, "a live probe must retain its slot beyond the cache TTL");
      ...
      assert.equal(probes - before, 1, "time spent waiting for PostgreSQL must not consume the completed TTL");
      await pause(400);

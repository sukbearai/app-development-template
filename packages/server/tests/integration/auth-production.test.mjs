...
const run = (source, extra = {}) =>
  execute(process.execPath, ["--import", "tsx", "--input-type=module", "--eval", source], {
    cwd: path.join(workspace, "packages/server"),
    env: { ...process.env, NODE_ENV: "test", ...extra },
  });
...
test("HTTP password schemas preserve the bootstrap and new-user credential bytes", async () => {
  ...
  assert.equal(
    await verifyPassword(
      loginRequestSchema.parse({ account: " admin ", password }).password,
      encoded,
    ),
    true,
  );
  assert.equal(
    createUserRequestSchema.parse({ account: "user", displayName: "User", password }).password,
    password,
  );
...
test("multiple Web processes require a shared limiter", async () => {
  await assert.rejects(
    run("await import('./src/env.ts')", { WEB_REPLICAS: "2", RATE_LIMIT_DRIVER: "memory" }),
    /RATE_LIMIT_DRIVER=redis/,
  );
});

test(
  "independent Redis clients share an overall login budget and fail closed",
  { timeout: 60000 },
  async () => {
    ...
    try {
      for (let i = 0; i < 40; i++) {
        try {
          if (docker("exec", name, "redis-cli", "ping") === "PONG") break;
        } catch {}
        await pause(100);
      }
      ...
      const config = {
        RATE_LIMIT_DRIVER: "redis",
        WEB_REPLICAS: "2",
        REDIS_URL: `redis://127.0.0.1:${port}/7`,
        LOGIN_RATE_LIMIT_GLOBAL_MAX: "4",
      };
      const source = `
...
test(
  "password lifecycle commits hashes, revocations, audit and outbox atomically",
  { timeout: 120000 },
  async (t) => {
    ...
    try {
      ...
      await t.test(
        "every rejected account performs password work without creating a session",
        async () => {
          const disabled = await auth.createManagedUser(
            {
          ...
          for (const legacy of [false, true]) {
            if (legacy)
              await query(
                "update app_users set password_hash='plain:disabled-password' where id=$1",
                [disabled.id],
              );
            for (const attempt of legacy ? attempts.slice(2) : attempts) {
              ...
              const hook = createHook({
                init(_id, type) {
                  if (type === "SCRYPTREQUEST") passwordJobs++;
                },
              });
              hook.enable();
              try {
              ...
              assert.equal(
                passwordJobs,
                1,
                `${attempt.account} must perform one password derivation`,
              );
            }
          }
          assert.equal(
            (await query("select count(*) from app_user_sessions")).rows[0].count,
            before,
          );
        },
      );
      ...
      await t.test("historical long credentials can log in and rotate", async () => {
        ...
        const legacy = await auth.createManagedUser(
          {
            account: "legacy-long",
            displayName: "Legacy",
        ...
        await auth.changePassword(
          { currentPassword: oldPassword, newPassword: "current-valid-password" },
          token,
        );
        await assert.rejects(auth.getCurrentUser(token), (error) => error.status === 401);
        assert.equal(
          (await auth.login({ account: legacy.account, password: "current-valid-password" })).user
            .id,
          legacy.id,
        );
      });
      ...
      await t.test(
        "denied reset attempts cannot change credentials or revoke sessions",
        async () => {
          await assert.rejects(
            auth.resetManagedUserPassword(
              admin.id,
          ...
          await assert.rejects(
            ...
            (e) => e.code === "CURRENT_PASSWORD_REQUIRED",
          );
          await assert.rejects(
            auth.changePassword(
              { currentPassword: "wrong", newPassword: "replacement-value" },
              userToken,
            ),
            (e) => e.code === "INVALID_CURRENT_PASSWORD",
          );
      ...
      await t.test(
        "concurrent rotations accept one replacement and invalidate every old session",
        async () => {
          const otherToken = (await auth.login({ account: user.account, password: userPassword }))
            .token;
          const attempts = await Promise.allSettled(
            [" replacement-one ", " replacement-two "].map((newPassword) =>
              auth.changePassword(
          ...
          for (const token of [userToken, otherToken])
            await assert.rejects(auth.getCurrentUser(token), (e) => e.status === 401);
          await assert.rejects(
            auth.login({ account: user.account, password: userPassword }),
            (e) => e.status === 401,
          );
          const winner =
            attempts[0].status === "fulfilled" ? " replacement-one " : " replacement-two ";
          userToken = (await auth.login({ account: user.account, password: winner })).token;
          const rows = (
            await query(
              "select * from app_audit_logs where action='auth.password.change' and target_id=$1",
              [user.id],
            )
          ).rows;
          ...
          assert.equal(
            (
              await query("select count(*) from app_outbox_events where payload->>'auditId'=$1", [
                rows[0].id,
              ])
            ).rows[0].count,
            "1",
      ...
      await t.test("outbox failure rolls back password replacement and revocation", async () => {
        const before = (await query("select password_hash from app_users where id=$1", [user.id]))
          .rows[0];
        await query(
          "create function reject_auth_outbox() returns trigger language plpgsql as $$ begin raise exception 'isolated outbox fault'; end $$; create trigger reject_auth_outbox before insert on app_outbox_events for each row execute function reject_auth_outbox()",
        );
        try {
          await assert.rejects(
            auth.resetManagedUserPassword(
              user.id,
              { newPassword: "failed-replacement" },
              adminToken,
            ),
          );
        } finally {
          await query(
            "drop trigger reject_auth_outbox on app_outbox_events; drop function reject_auth_outbox()",
          );
        }
        assert.deepEqual(
          (await query("select password_hash from app_users where id=$1", [user.id])).rows[0],
          before,
        );
        assert.equal((await auth.getCurrentUser(userToken)).user.id, user.id);
      });
      ...
      await t.test(
        "administrator reset revokes stale sessions; revoked actors cannot reset",
        async () => {
          assert.deepEqual(
            await auth.resetManagedUserPassword(
              user.id,
          ...
          await assert.rejects(
            auth.resetManagedUserPassword(user.id, { newPassword: "unauthorized-reset" }, stale),
            (e) => e.status === 401,
          );
        },
      );
      ...
      await t.test(
        "reset rechecks administrator rights after hashing and waiting for the identity lock",
        async () => {
          await auth.createManagedRole(
            { id: "resetter", name: "Resetter", status: "active", permissionIds: ["admin.write"] },
            adminToken,
          );
          const resetter = await auth.createManagedUser(
            {
              account: "resetter",
          ...
          const resetterToken = (
            await auth.login({ account: resetter.account, password: "resetter-password" })
          ).token;
          ...
          try {
            ...
            attempt = auth.resetManagedUserPassword(
              user.id,
              { newPassword: "must-never-be-set" },
              resetterToken,
            );
      ...
      await t.test(
        "operator CLI requires confirmation and recovers modern administrator without leaking credentials",
        async () => {
          ...
          await assert.rejects(
            execute(
              ...
              [
                "--import",
                "tsx",
                "scripts/recover-admin.ts",
                "--account",
                adminCredentials.account,
              ],
              { cwd: path.join(workspace, "packages/server"), env: cliEnv },
            ),
            (e) => !`${e.stdout}${e.stderr}`.includes(replacement),
          );
          ...
          adminToken = (
            await auth.login({ account: adminCredentials.account, password: replacement })
          ).token;
          await assert.rejects(
          ...
          const row = (
            await query("select * from app_audit_logs where action='admin.credentials.recovered'")
          ).rows[0];

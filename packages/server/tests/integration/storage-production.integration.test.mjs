  ...
  async (t) => {
    ...
    try {
      ...
      const auth = await import("../../src/modules/identity/service.ts"),
        product = await import("../../src/modules/uploads/service.ts"),
        storage = await import("../../src/storage.ts");
      storageClient = await import("../../src/s3-client.ts");
      const repo_retention = await import("@pstack/database/retention"),
        repo_runtime = await import("@pstack/database/modules/runtime/repository");
      ...
      await t.test(
        ...
        async () => {
          ...
          const preview = await repo_retention.runRetention(options);
          assert.equal(preview.telemetry, 2);
          assert.equal(preview.idempotency, 1);
          assert.equal(preview.taskEvents, 1);
          assert.deepEqual(
            await repo_retention.runRetention({ ...options, dryRun: false }),
            preview,
          );
          const [receipt] = (
          ...
          assert.equal(
            (await repo_retention.runRetention({ ...options, dryRun: false })).telemetry,
            1,
          );
        },
      );
      await t.test("health returns aggregated buckets with real counts", async () => {
        const rows = await repo_runtime.getAsyncRuntimeHealthRows();

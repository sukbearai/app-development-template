...
export async function recoverAdministrator(input: z.input<typeof recoveryInputSchema>) {
  ...
  return withTransaction(async (tx) => {
    ...
    const permissions = new Set(roles.filter((role) => role.status === "active" && current?.user.roleIds.includes(role.id)).flatMap((role) => role.permissionIds));
    if (!current || current.user.status !== "enabled" || !permissions.has("admin.read") || !permissions.has("admin.write"))
    ...
    await recordAudit({ actorId: current.user.id, action: "admin.credentials.recovered", targetType: "user", targetId: current.user.id, traceId: "operator-recovery", metadata: { channel: "operator-cli" } }, tx);

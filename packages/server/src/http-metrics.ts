...
const counters = new Map(apiOperations.filter((operation) => operation.operationId !== "getApiSystemMetrics")
  .flatMap((operation) => Object.keys(operation.responses).map((status) => {
    const metric: RuntimeMetrics["http"][number] = {
      operationId: operation.operationId, status: Number(status), count: 0, durationMsTotal: 0, durationMsMax: 0,
    };

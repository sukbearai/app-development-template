import {
  ...
  readRuntimeHealthConfiguration,
} from "../../runtime-health-config";
import * as repo from "@pstack/database/modules/runtime/repository";
import type { AdminSummary } from "@pstack/contracts/modules/runtime/contracts";
import {
...
import { getAsyncRuntimeHealthRows } from "@pstack/database/modules/runtime/repository";
...
export function buildAsyncRuntimeHealthSnapshot(input: {
  ...
  const staleLockMs = input.staleLockMs ?? readRuntimeHealthConfiguration().staleLockMs;
  ...
  alerts.push(
    ...evaluateOutboxBacklog(
      backlog,
      input.thresholds ?? readRuntimeHealthConfiguration().thresholds,
    ),
  );
...
export async function readAdminAsyncRuntimeHealth() {
  const now = new Date();
  const staleLockMs = readRuntimeHealthConfiguration().staleLockMs;

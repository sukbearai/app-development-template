...
const ownerPath = production ? process.env.PSTACK_VERIFY_OWNER : `${root}/apps/web/.vinext/dev/lock.json`;
...
if (production) {
  ...
  assert.equal(lock.startedAt, Number(process.env.PSTACK_VERIFY_STARTED_MS), 'Owner belongs to another run');
  assert.deepEqual(processIdentity(lock.pid), { ppid: lock.harnessPid, pgid: lock.pid, processStarted: lock.processStarted }, 'Production child identity changed');
  ...
  assert.equal(ancestor, lock.harnessPid, 'Doctor must run under the harness that owns this server');
  const cwd = execFileSync('lsof', ['-a', '-p', String(lock.pid), '-d', 'cwd', '-Fn'], { encoding: 'utf8' });
  assert.ok(cwd.split('\n').includes(`n${root}/apps/web`), 'Production child belongs to another application root');
...
assert.deepEqual([...new Set(listeners)], [String(lock.pid)], 'The owner must exclusively own the listening port');
...
console.log(JSON.stringify({ status: 'ready', mode: production ? 'production' : 'development', root, lock, node: process.version, vinext: pkg.dependencies.vinext, baseURL: `http://127.0.0.1:${port}` }, null, 2));

...
`test:e2e` exercises API persistence and rejection paths. `test:ui` exercises login, pre-hydration submission rejection, role and user creation, status changes, password reset/rotation, upload, file pagination, navigation and logout. `test:production` builds and serves the production artifact through the application launcher, then runs API checks before and after database restore. `node scripts/verify-app.mjs --production --ui` also runs the same eight Playwright flows before and after restore. Both production commands verify graceful Web shutdown, including an upload whose client disconnected and a forced timeout. Redis, Kafka, S3 and backup recovery have separate integration tests; passing the browser checks does not prove those services.
